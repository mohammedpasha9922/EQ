...
function smartPdfSyncCount() {
  const countEl = smartPdfEl('smartPdfPageCount');
  if (!countEl) return;
  if (!smartPdfState.totalPages) { countEl.textContent = ''; return; }
  countEl.textContent = String(smartPdfT('smartPdfPagesLabel', '{n} pages')).replace('{n}', String(smartPdfState.totalPages));
}
