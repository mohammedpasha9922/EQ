...
function updateConverterOutput() {
  ...
  if (currencyWords && result !== 0) {
    currencyWords.textContent = numberToWords(displayResult, state.locale);
...
function handleCurrencyAction(action) {
  ...
  switch (action) {
    case 'search':
      // Global Currency Directory & Search (existing search logic)
    ...
    case 'convert':
      ...
      break;
    case 'favorites':
      // Open the existing converter then surface the existing favorites list
      openCurrencyConverter();
      if (currencyServiceInstance) {
        setTimeout(() => showFavoritesList(), 60);
      }
      break;
  }
}
