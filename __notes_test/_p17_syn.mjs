...
function smartScanBuildEditDoc() {
  ...
  items.forEach((it) => {
    if (it.kind === 'heading') blocks.push({ kind: 'heading', level: 1, text: it.block.text });
      ...
      const rows = (it.tb.rows || []).map((r) => (Array.isArray(r) ? r : []).map((cell) => ({ text: String((cell && cell.text) || '') })));
...
function smartScanReadEditDocFromDom() {
  ...
  if (state.smartScanResult) state.smartScanResult.editedDoc = smartScanEditDoc;
}
