...
function smartPdfOverlayBox(page, o, kindCls, innerHTML, editable) {
 ...
 if (editable) {
   body.addEventListener('input', () => { const oo = smartPdfOverlayFind(page, o.id); if (oo && kindCls === 'table') oo.rows = smartPdfOverlayReadTable(box); else if (oo && (kindCls === 'text' || kindCls === 'date')) oo.text = (body.textContent || '');; });
 ...
 box.addEventListener('pointerdown', (ev) => {
   ...
   if (kindCls === 'table' && ev.target && ev.target.closest && ev.target.closest('td[contenteditable]')) return;
   ev.preventDefault();
