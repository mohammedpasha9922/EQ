...
<body>
  <div class="eq-note-report${tplClass}${pdfStyleClass}${pdfFrameClass}" id="note-report">
    ...
    <div class="eq-note-header">
      ...
      ${showHeader ? '<div class="eq-pdf-header-note" dir="' + titleDir + '">' + escapeHtml(title) + '</div>' : ''}
...
function serializeNoteEditor() {
  ...
  if (!hasTable && !hasChecklist && !hasDivider) return extractNoteBodyAndFormatting(root);
  const blocks = [];
  let textBuf = [];
  const flush = () => {
  ...
  flush();
  if (!blocks.some((b) => b.type === 'table' || b.type === 'checklist' || b.type === 'divider')) return extractNoteBodyAndFormatting(root);
...
function openFullScreenNote(noteId) {
  ...
  if (noteBodyInput) {
    ...
    noteBodyInput.classList.remove('note-frame-none', 'note-frame-classic', 'note-frame-soft', 'note-frame-shadow');
...
function wireEvents() {
  ...
  if (noteAaBtn && noteAaPanel) {
    ...
    function applyNoteStyleClass(styleId) {
      ...
      if (styleId) el.classList.add('note-style-' + styleId);
    }
