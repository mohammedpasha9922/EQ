...
const translations = {
  ...
  pdfTplBlank: 'Blank', pdfTplReport: 'Report', pdfTplInvoice: 'Invoice', pdfTplReceipt: 'Receipt', pdfTplContract: 'Contract', pdfTplCv: 'CV', pdfTplBusiness: 'Business Report', pdfTplEngineering: 'Engineering Report', pdfTplLetter: 'Letter',
    ...
    smartPdfAdd: 'Add', smartPdfAddText: 'Text', smartPdfAddImage: 'Image', smartPdfAddLogo: 'Logo', smartPdfAddSignature: 'Signature', smartPdfAddStamp: 'Stamp', smartPdfAddDate: 'Date', smartPdfAddTable: 'Table',
...
smartScanReviewNote: 'Documento reconocido. Revisa el contenido antes de crear el PDF.',
  ...
  pdfTplBlank: 'فارغ', pdfTplReport: 'تقرير', pdfTplInvoice: 'فاتورة', pdfTplReceipt: 'إيصال', pdfTplContract: 'عقد', pdfTplCv: 'سيرة ذاتية', pdfTplBusiness: 'تقرير أعمال', pdfTplEngineering: 'تقرير هندسي', pdfTplLetter: 'رسالة',
    ...
    smartPdfAdd: 'إضافة', smartPdfAddText: 'نص', smartPdfAddImage: 'صورة', smartPdfAddLogo: 'شعار', smartPdfAddSignature: 'توقيع', smartPdfAddStamp: 'ختم', smartPdfAddDate: 'تاريخ', smartPdfAddTable: 'جدول',
...
function smartPdfOverlayBox(page, o, kindCls, innerHTML, editable) {
 ...
 grip.addEventListener('pointerdown', (ev) => {
   ...
   const onMove = (e2) => {
     const s = smartPdfOverlayScale(); if (!s) return;
     o.w = Math.max(40, Math.min(600, ow + (e2.clientX - startX) / s));
 ...
 delbtn.addEventListener('click', (ev) => { ev.preventDefault(); ev.stopPropagation(); smartPdfOverlayRemove(page, o.id); smartPdfRenderOverlays(); });
 return box;
}
 ...
 for (const pk of pages) {
   ...
   list.forEach((o) => {
     ...
     switch (o.type) {
       ...
       case 'table': smartPdfOverlayBox(page, o, 'table', smartPdfOverlayTableHtml(o.rows || [['', ''], ['', '']]), true); break;
     }
   });
