...
const translations = {
  ...
  pdfTplBlank: 'Blank', pdfTplReport: 'Report', pdfTplInvoice: 'Invoice', pdfTplReceipt: 'Receipt', pdfTplContract: 'Contract', pdfTplCv: 'CV', pdfTplBusiness: 'Business Report', pdfTplEngineering: 'Engineering Report', pdfTplLetter: 'Letter',
    ...
    smartPdfAdd: 'Add', smartPdfAddText: 'Text', smartPdfAddImage: 'Image', smartPdfAddLogo: 'Logo', smartPdfAddSignature: 'Signature', smartPdfAddStamp: 'Stamp', smartPdfAddDate: 'Date', smartPdfAddTable: 'Table', pdfTblRow: 'Add Row', pdfTblRowDel: 'Delete Row', pdfTblCol: 'Add Column', pdfTblColDel: 'Delete Column', pdfTblAlignL: 'Left', pdfTblAlignC: 'Center', pdfTblAlignR: 'Right', pdfTblBold: 'Bold', pdfTblItalic: 'Italic', pdfTblTextColor: 'Text Color', pdfTblBg: 'Background', pdfTblBorder: 'Border Color', pdfTblNoBorder: 'No border', pdfTblRowH: 'Row height', pdfTblControls: 'Table controls',
...
smartScanReviewNote: 'Documento reconocido. Revisa el contenido antes de crear el PDF.',
  ...
  pdfTplBlank: 'فارغ', pdfTplReport: 'تقرير', pdfTplInvoice: 'فاتورة', pdfTplReceipt: 'إيصال', pdfTplContract: 'عقد', pdfTplCv: 'سيرة ذاتية', pdfTplBusiness: 'تقرير أعمال', pdfTplEngineering: 'تقرير هندسي', pdfTplLetter: 'رسالة',
    ...
    smartPdfAdd: 'إضافة', smartPdfAddText: 'نص', smartPdfAddImage: 'صورة', smartPdfAddLogo: 'شعار', smartPdfAddSignature: 'توقيع', smartPdfAddStamp: 'ختم', smartPdfAddDate: 'تاريخ', smartPdfAddTable: 'جدول', pdfTblRow: 'إضافة صف', pdfTblRowDel: 'حذف صف', pdfTblCol: 'إضافة عمود', pdfTblColDel: 'حذف عمود', pdfTblAlignL: 'يسار', pdfTblAlignC: 'وسط', pdfTblAlignR: 'يمين', pdfTblBold: 'عريض', pdfTblItalic: 'مائل', pdfTblTextColor: 'لون النص', pdfTblBg: 'الخلفية', pdfTblBorder: 'لون الحدود', pdfTblNoBorder: 'بدون حدود', pdfTblRowH: 'ارتفاع الصف', pdfTblControls: 'أدوات الجدول',
...
function smartScanOpen() {
  ...
  if (view) {
    view.classList.add('scan-visible');
    view.setAttribute('aria-hidden', 'false');
    // PART 17 — on narrow viewports the Smart Documents modal content is wider
    // than the screen (720px layout); make sure the scan view itself is the
    // visible part instead of being scrolled out of the viewport.
    try {
      if (smartDocsModal && typeof view.scrollIntoView === 'function') {
        view.scrollIntoView({ inline: 'start', block: 'start' });
      }
    } catch (e) { /* non-fatal */ }
  }
  setSmartDocsStep(1);
...
function smartScanUpdateReviewStatus() {
  const el = scanEl('scanReviewStatus');
  if (!el) return;
  const t = smartScanT();
  const edited = !!(state && state.smartScanResult && state.smartScanResult.editedDoc && state.smartScanResult.userEdited);
  if (edited) {
    el.setAttribute('data-state', 'edited');
    el.textContent = t.smartScanStatusEdited || 'Edited — your corrections will be used in the PDF';
  } else {
    el.setAttribute('data-state', 'needs');
    el.textContent = (t.smartScanReviewNote || 'Document recognized. Review the content before creating the PDF.');
  }
}

function smartScanMarkEdited() {
  if (state) { state.smartScanResult = state.smartScanResult || {}; state.smartScanResult.userEdited = true; }
  if (typeof smartScanUpdateReviewStatus === 'function') smartScanUpdateReviewStatus();
}
...
function bindSmartScanEdit() {
  ...
  const blocksEl = scanEl('scanEditBlocks');
  if (blocksEl) {
    blocksEl.addEventListener('input', () => { smartScanReadEditDocFromDom(); smartScanMarkEdited(); });
    blocksEl.addEventListener('click', (e) => {
      const btn = (e.target && e.target.closest) ? e.target.closest('[data-action]') : null;
      if (btn) { smartScanEditStructural(btn.dataset.action, parseInt(btn.dataset.bi, 10)); smartScanMarkEdited(); }
    });
  }
  const titleEl = scanEl('scanEditDocTitle');
  if (titleEl) titleEl.addEventListener('input', () => { if (smartScanEditDoc) smartScanEditDoc.title = titleEl.value; if (state.smartScanResult) state.smartScanResult.editedDoc = smartScanEditDoc; smartScanMarkEdited(); });
  document.querySelectorAll('[data-add-block]').forEach((btn) => {
    btn.addEventListener('click', () => { smartScanEditAdd(btn.getAttribute('data-add-block')); smartScanMarkEdited(); });
  });
...
// PART 21 — added-text overlay annotations (session-editable). Each overlay is an
// EDITABLE text note laid over the page as an HTML layer (move/resize/delete/edit). It is
// exported into the PDF at Save via pdf-lib drawText (REAL added text, original pdf
// untouched — honest "editable overlay" per PART 19 §6. Overlays are NOT original-text.
// { pageIndex: [ { id, x, y, size, text, align } ] } in PDF points (page coordinates.
let smartImportOverlays = null;
let smartImportOverlaySeq = 0;
// PART 22 — current PDF page being edited (0-based; used so added elements land on
// the page the user is actually looking at,and each element keeps its page association).
let smartImportCurrentPage = 0;
...
function smartImportResetUi() {
  smartImportParsed = null; smartImportOcrOverride = null; smartImportEdits = null; smartImportColors = null; smartImportOverlays = null;
...
async function smartPdfEditorBuild() {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    // PART 22 — track the page being viewed/edited so added elements land on it.
    wrap.addEventListener('pointerdown', () => { smartImportCurrentPage = i; }, { passive: true });
  ...
  // PART 22 — render any existing added elements (date/text/image/logo/signature/
  // stamp/table) onto their own pages.
...
// ============================================================
// PART 22 — PDF Add (Smart Docs PDF Editor only)
// An Add menu (Text/Image/Logo/Signature/Stamp/Date/Table) inserts EDITABLE,
// page-associated elements onto the current PDF page. Each lands in smartImportOverlays
// (session-local, PDF-only — never in Notes/bodyBlocks) and on Save is drawn into the REAL
// PDF via pdf-lib (drawText/drawImage/drawRectangle) with the original preserved (safeBytes,
// new blob; no screenshot/flattening). Overlays are PDF-only and NOT original text.
function smartPdfPageDims(i) {
  const pages = (smartImportParsed && smartImportParsed.pages) || [];
  const m = pages[i]; return (m && m.dims && m.dims.width) ? m.dims.width : 0;
}
function smartPdfOverlayTarget() {
  const holder = smartImportEl('smartPdfEditor');
  if (!holder) return null;
  const wrap = holder.querySelector('.smart-pdf-page[data-page="' + smartImportCurrentPage + '"]');
 if (!wrap) return { holder, wrap: null };
 // a dedicated layer above the pdf.js text layer, inside the page box.

  let layer = wrap.querySelector(':scope > .smart-pdf-overlay-layer');
  if (!layer) { layer = document.createElement('div'); layer.className = 'smart-pdf-overlay-layer'; wrap.appendChild(layer); }
 return { holder, wrap, layer };
}
function smartPdfOverlayScale() {
  if (typeof document === 'undefined') return 1;

  const holder = smartImportEl('smartPdfEditor');
  const wrap = holder ? holder.querySelector('.smart-pdf-page[data-page="' + smartImportCurrentPage + '"]') : null;
 if (!wrap) return 1;
 try { const w = wrap.clientWidth; if (!w) return 1; const d = smartPdfPageDims(smartImportCurrentPage); return d ? w / d : 1; } catch (e) { return 1; }
}
function smartPdfOverlayStore(page, o)
{
  if (!smartImportOverlays) smartImportOverlays = {};
 if (!smartImportOverlays[page]) smartImportOverlays[page] = [];
 const list = smartImportOverlays[page];
 const i = list.findIndex((x) => x && x.id === o.id);
 if (i >= 0) list[i] = o; else list.push(o);
}
function smartPdfOverlayRemove(page, id)
{
  if (!smartImportOverlays || !smartImportOverlays[page]) return;
 smartImportOverlays[page] = smartImportOverlays[page].filter((x) => !x || x.id !== id);
}
function smartPdfOverlayFind(page, id)
{
  return smartImportOverlays && smartImportOverlays[page] && smartImportOverlays[page].find((x) => x && x.id === id) || null;
}
function smartPdfEscHtml(s)
{
...
function smartPdfOverlayTableHtml(rows)
{
  const ar = Array.isArray(rows) ? rows : [['', ''], ['', '']];
 let h = '<table><tbody>';
 ar.forEach((r) => { h += '<tr>'; (Array.isArray(r) ? r : []).forEach((c) => { h += '<td contenteditable="true">' + smartPdfEscHtml(c) + '</td>'; }); h += '</tr>'; });
 return h + '</tbody></table>';
}
function smartPdfOverlayReadTable(box, prevRows) {
...
// Build a positioned, movable, resizable, deletable, editable overlay box.
// PART 23 — PDF table model (structured rows × cell objects). The table overlay
// stays a JS data model — never a screenshot/bitmap — and the SAME model drives
// the editor table, the preview and the final pdf-lib export (drawText + lines).
function smartPdfCellObj(v) {
  if (v && typeof v === 'object') {
    return { text: String(v.text == null ? '' : v.text), align: (v.align === 'center' || v.align === 'right' || v.align === 'left') ? v.align : '', bg: (typeof v.bg === 'string' && v.bg) || '', color: (typeof v.color === 'string' && v.color) || '', bold: !!v.bold, italic: !!v.italic };
  }
  return { text: String(v == null ? '' : v), align: '', bg: '', color: '', bold: false, italic: false };
}
function smartPdfTableNormalize(rows) {
  const src = Array.isArray(rows) && rows.length ? rows : [['', ''], ['', '']];
  return src.map((r) => (Array.isArray(r) ? r : [r]).map(smartPdfCellObj));
}
// PART 23 — css color (#hex or rgb()) -> pdf-lib rgb() components (export path).
function smartPdfPdfColorP23(css) {
  const s = String(css || '').trim();
  let m = /^#?([0-9a-f]{6})$/i.exec(s);
  if (m) { const v = parseInt(m[1], 16); return { r: ((v >> 16) & 255) / 255, g: ((v >> 8) & 255) / 255, b: (v & 255) / 255 }; }
  m = /rgba?\(([^)]+)\)/.exec(s);
  if (m) { const p = m[1].split(',').map((z) => parseFloat(z)); return { r: (p[0] || 0) / 255, g: (p[1] || 0) / 255, b: (p[2] || 0) / 255 }; }
  return { r: 0.1, g: 0.12, b: 0.16 };
}
// Cell markup for the EDITOR table (contenteditable td = direct cell editing).
function smartPdfCellHtmlP23(c, ri, ci, pc) {
  let st = '';
  if (c.bg) st += 'background:' + c.bg + ';';
  if (c.color) st += 'color:' + c.color + ';';
  if (c.align) st += 'text-align:' + c.align + ';direction:auto;';
  if (c.bold) st += 'font-weight:700;';
  if (c.italic) st += 'font-style:italic;';
  const bcls = pc.hasBorder === false ? ' smart-pdf-td-nob' : '';
  return '<td contenteditable="true" spellcheck="false" data-r="' + ri + '" data-c="' + ci + '" class="smart-pdf-td' + bcls + '"' + (st ? ' style="' + smartPdfEscAttr(st) + '"' : '') + '>' + smartPdfEscHtml(c.text) + '</td>';
}
function smartPdfOverlayTableHtmlP23(rows, pc) {
  const p23 = smartPdfTableNormalize(rows);
  const opt = pc || {};
  const fs = opt.size ? 'font-size:' + opt.size + 'px;' : '';
  const bw = (typeof opt.borderWidth === 'number' && opt.borderWidth > 0) ? opt.borderWidth : 1;
  const bc = opt.borderColor || 'rgba(15, 23, 42, 0.45)';
  let h = '<table class="smart-pdf-table"' + (fs || (opt.hasBorder === false) ? ' style="' + smartPdfEscAttr((fs || '') + ((opt.hasBorder === false) ? '--tdbw:0px;' : '--tdbw:' + bw + 'px;--tdbc:' + bc + ';')) + '"' : '') + '>';
  for (let r = 0; r < p23.length; r++) {
    h += '<tr>';
    for (let c = 0; c < p23[r].length; c++) h += smartPdfCellHtmlP23(p23[r][c], r, c, opt);
    h += '</tr>';
  }
  return h + '</table>';
}
// Structured-table probes used by the export path (keeps the model as source of truth).
function smartPdfTableRectsP23(o, x, y, w, h) {
  const rows = smartPdfTableNormalize(o.rows);
  const cols = rows[0].length;
  const cw = w / cols, rh = h / rows.length;
  const rects = [];
  for (let r = 0; r < rows.length; r++) for (let c = 0; c < rows[r].length; c++) {
    const cell = rows[r][c];
    if (cell && cell.bg && cell.bg.toUpperCase() !== '#FFFFFF') rects.push({ x: x + c * cw, y: y + r * rh, w: cw, h: rh, fill: cell.bg });
  }
  return rects;
}
function smartPdfTableBorderSegsP23(o, x, y, w, h) {
  if (o.hasBorder === false) return [];
  const rows = smartPdfTableNormalize(o.rows);
  const cols = rows[0].length;
  const cw = w / cols, rh = h / rows.length, segs = [];
  for (let r = 0; r <= rows.length; r++) segs.push({ x1: x, y1: y + r * rh, x2: x + w, y2: y + r * rh });
  for (let c = 0; c <= cols; c++) segs.push({ x1: x + c * cw, y1: y, x2: x + c * cw, y2: y + h });
  return segs;
}

// PART 23 — table controls bar (rows/columns/colors/borders/alignment/height).
// Reuses the EXISTING color palette ids from the Add-menu color system; the bar
// writes into the SAME structured table model that the export path renders from.
function smartPdfTableBarBtnP23(label, title, act) {
  return '<button type="button" class="smart-pdf-tbtn" data-tact="' + act + '" aria-label="' + smartPdfEscAttr(title) + '" title="' + smartPdfEscAttr(title) + '">' + smartPdfEscHtml(label) + '</button>';
}
function smartPdfTableSwatchP23(color, act, title) {
  return '<button type="button" class="smart-pdf-tswatch" data-tact="' + act + '" data-color="' + smartPdfEscAttr(color) + '" style="background:' + smartPdfEscAttr(color) + '" aria-label="' + smartPdfEscAttr(title) + '" title="' + smartPdfEscAttr(title) + '"></button>';
}
function smartPdfTableApplyP23(page, id, fn) {
  const oo = smartPdfOverlayFind(page, id);
  if (!oo) return;
  oo.rows = smartPdfTableNormalize(oo.rows);
  if (fn) fn(oo);
  smartPdfOverlayStore(page, oo);
  smartPdfRenderOverlays();
}
function smartPdfTableToolbarHtmlP23() {
  const t = smartImportT();
  let h = smartPdfTableBarBtnP23('+', t.pdfTblRow || 'Add Row', 'row+') + smartPdfTableBarBtnP23('−', t.pdfTblRowDel || 'Delete Row', 'row-');
  h += smartPdfTableBarBtnP23('+', t.pdfTblCol || 'Add Column', 'col+') + smartPdfTableBarBtnP23('−', t.pdfTblColDel || 'Delete Column', 'col-');
  h += smartPdfTableBarBtnP23('⇤', t.pdfTblAlignL || 'Left', 'al-l') + smartPdfTableBarBtnP23('≡', t.pdfTblAlignC || 'Center', 'al-c') + smartPdfTableBarBtnP23('⇥', t.pdfTblAlignR || 'Right', 'al-r');
  h += smartPdfTableBarBtnP23('B', t.pdfTblBold || 'Bold', 'f-b') + smartPdfTableBarBtnP23('I', t.pdfTblItalic || 'Italic', 'f-i');
  h += smartPdfTableSwatchP23('#000000', 'tc', t.pdfTblTextColor || 'Text Color') + smartPdfTableSwatchP23('#ff0000', 'tc', t.pdfTblTextColor || 'Text Color') + smartPdfTableSwatchP23('#008000', 'tc', t.pdfTblTextColor || 'Text Color') + smartPdfTableSwatchP23('#0000ff', 'tc', t.pdfTblTextColor || 'Text Color');
  h += smartPdfTableSwatchP23('#ffff00', 'bg', t.pdfTblBg || 'Background') + smartPdfTableSwatchP23('#ffa500', 'bg', t.pdfTblBg || 'Background') + smartPdfTableSwatchP23('#800080', 'bg', t.pdfTblBg || 'Background') + smartPdfTableSwatchP23('#ffffff', 'bg', t.pdfTblBg || 'Background');
  h += smartPdfTableSwatchP23('#000000', 'bc', t.pdfTblBorder || 'Border Color') + smartPdfTableSwatchP23('#ff0000', 'bc', t.pdfTblBorder || 'Border Color') + smartPdfTableSwatchP23('#0000ff', 'bc', t.pdfTblBorder || 'Border Color');
  h += smartPdfTableBarBtnP23(t.pdfTblNoBorder || 'No border', t.pdfTblNoBorder || 'No border', 'bn') + smartPdfTableBarBtnP23(t.pdfTblBorder || 'Border', t.pdfTblBorder || 'Border', 'by');
  h += smartPdfTableBarBtnP23('H−', t.pdfTblRowH || 'Row height', 'rh-') + smartPdfTableBarBtnP23('H+', t.pdfTblRowH || 'Row height', 'rh+');
  return h;
}
function smartPdfAttachTableBarP23(box, page, o) {
  const bar = document.createElement('div');
  bar.className = 'smart-pdf-tbar';
  bar.setAttribute('role', 'toolbar');
  bar.setAttribute('aria-label', smartImportT().pdfTblControls || 'Table controls');
  bar.innerHTML = smartPdfTableToolbarHtmlP23();
  bar.addEventListener('pointerdown', (ev) => ev.stopPropagation());
  bar.addEventListener('click', (ev) => {
    const b = ev.target && ev.target.closest ? ev.target.closest('[data-tact]') : null;
    if (!b) return;
    ev.preventDefault(); ev.stopPropagation();
    const act = b.getAttribute('data-tact');
    const sel = box.querySelector('td.is-sel');
    const id = o.id;
    if (act === 'row+') smartPdfTableApplyP23(page, id, (oo) => { const cols = (oo.rows[0] || []).length || 1; oo.rows.push(Array.from({ length: cols }, () => smartPdfCellObj(''))); });
    else if (act === 'row-') smartPdfTableApplyP23(page, id, (oo) => { if (oo.rows.length > 1) oo.rows.pop(); });
    else if (act === 'col+') smartPdfTableApplyP23(page, id, (oo) => { oo.rows.forEach((r) => r.push(smartPdfCellObj(''))); });
    else if (act === 'col-') smartPdfTableApplyP23(page, id, (oo) => { if ((oo.rows[0] || []).length > 1) oo.rows.forEach((r) => r.pop()); });
    else if (act === 'rh-') smartPdfTableApplyP23(page, id, (oo) => { oo.rowH = Math.max(12, Math.round((Number(oo.rowH) || 14) - 4)); if (oo.rowH <= 12) delete oo.rowH; });
    else if (act === 'rh+') smartPdfTableApplyP23(page, id, (oo) => { oo.rowH = Math.min(96, Math.round((Number(oo.rowH) || 14) + 4)); });
    else if (act === 'bn') smartPdfTableApplyP23(page, id, (oo) => { oo.hasBorder = false; });
    else if (act === 'by') smartPdfTableApplyP23(page, id, (oo) => { oo.hasBorder = true; });
    else if (act === 'al-l' || act === 'al-c' || act === 'al-r') {
      const al = act === 'al-l' ? 'left' : (act === 'al-c' ? 'center' : 'right');
      smartPdfTableApplyP23(page, id, (oo) => { if (sel) { const r0 = oo.rows[Number(sel.dataset.r)]; const c = r0 && r0[Number(sel.dataset.c)]; if (c) c.align = al; } else oo.rows.forEach((r) => r.forEach((c) => { c.align = al; })); });
    } else if (act === 'f-b' || act === 'f-i') {
      const k = act === 'f-b' ? 'bold' : 'italic';
      smartPdfTableApplyP23(page, id, (oo) => { if (sel) { const r0 = oo.rows[Number(sel.dataset.r)]; const c = r0 && r0[Number(sel.dataset.c)]; if (c) c[k] = !c[k]; } else oo.rows.forEach((r) => r.forEach((c) => { c[k] = !c[k]; })); });
    } else if (act === 'tc' || act === 'bg' || act === 'bc') {
      const color = b.getAttribute('data-color') || '#000000';
      if (act === 'bc') smartPdfTableApplyP23(page, id, (oo) => { oo.borderColor = color; oo.hasBorder = oo.hasBorder !== false; });
      else smartPdfTableApplyP23(page, id, (oo) => { if (sel) { const r0 = oo.rows[Number(sel.dataset.r)]; const c = r0 && r0[Number(sel.dataset.c)]; if (c) c[act === 'tc' ? 'color' : 'bg'] = color; } else oo.rows.forEach((r) => r.forEach((c) => { c[act === 'tc' ? 'color' : 'bg'] = color; })); });
    }
  });
  box.appendChild(bar);
  // PART 23 — clicking a cell must focus it for editing even when the overlay
  ...
  box.addEventListener('focusin', (ev) => {
    const td = ev.target && ev.target.closest ? ev.target.closest('td[contenteditable]') : null;
    if (!td) return;
    box.querySelectorAll('td.is-sel').forEach((x) => { if (x !== td) x.classList.remove('is-sel'); });
    td.classList.add('is-sel');
  });
}
function smartPdfOverlayBox(page, o, kindCls, innerHTML, editable) {
  const t = smartPdfOverlayTarget(); if (!t || !t.wrap || !t.layer) return;
  const scale = smartPdfOverlayScale();
  const box = document.createElement('div');
  box.className = 'smart-pdf-overlay smart-pdf-ov-' + kindCls;
  box.dataset.page = String(page); box.dataset.id = String(o.id);
  box.style.left = (o.x * scale) + 'px'; box.style.top = (o.y * scale) + 'px';
 box.style.width = ((o.w || 120) * scale) + 'px';
 if (kindCls === 'text' || kindCls === 'date') { box.style.fontSize = (o.size || 12) * scale + 'px'; }
 if (kindCls === 'table') { box.style.minWidth = (120 * scale) + 'px'; }
 const body = document.createElement(editable ? 'div' : 'span');
 body.className = 'smart-pdf-overlay-body';
 body.innerHTML = innerHTML;
 body.contentEditable = editable ? 'true' : 'false';
 box.appendChild(body);
 if (kindCls === 'table') smartPdfAttachTableBarP23(box, page, o);
 const grip = document.createElement('span'); grip.className = 'smart-pdf-overlay-grip'; grip.setAttribute('aria-label', 'Resize'); grip.title = 'Resize';
 box.appendChild(grip);
 const delbtn = document.createElement('button'); delbtn.type = 'button'; delbtn.className = 'smart-pdf-overlay-del'; delbtn.setAttribute('aria-label', 'Delete'); delbtn.textContent = '\u2715';
 box.appendChild(delbtn);
 t.layer.appendChild(box);
 // Live editing writes straight back into the store (authoritative; survives re-render).
 if (editable) {
       body.addEventListener('input', () => { const oo = smartPdfOverlayFind(page, o.id); if (oo && kindCls === 'table') oo.rows = smartPdfOverlayReadTable(box, oo.rows); else if (oo && (kindCls === 'text' || kindCls === 'date')) oo.text = (body.textContent || '');; });
   body.addEventListener('blur', () => { smartPdfRenderOverlays(); });  // normalize after edit
  }
 // move (drag anywhere on the box.
 box.addEventListener('pointerdown', (ev) => {
   if (ev.target === delbtn || ev.target === grip) return;
   // PART 23 — clicking a table cell must focus it for editing; only start a
   // drag from non-cell parts of the box (padding/borders/bar area).
   if (kindCls === 'table' && ev.target && ev.target.closest && ev.target.closest('td[contenteditable]')) {
   ...
   const ox = o.x, oy = o.y;
   const onMove = (e2) => {
     const s = smartPdfOverlayScale(); if (!s) return;
     o.x = Math.max(0, ox + (e2.clientX - startX) / s); o.y = Math.max(0, oy + (e2.clientY - startY) / s);
     box.style.left = (o.x * s) + 'px'; box.style.top = (o.y * s) + 'px';
   };
   const onUp = () => { window.removeEventListener('pointermove', onMove); window.removeEventListener('pointerup', onUp); smartPdfOverlayStore(page, o); };
 ...
 grip.addEventListener('pointerdown', (ev) => {
   ...
   const ow = (o.w || 120), osz = (o.size || 12);
   const onMove = (e2) => {
     const s = smartPdfOverlayScale(); if (!s) return;
     const maxW = (typeof window !== 'undefined' && window.__smartPdfTableMaxW) ? window.__smartPdfTableMaxW : 600;
     o.w = Math.max(40, Math.min(maxW, ow + (e2.clientX - startX) / s));
     if (kindCls === 'table') { o.size = Math.max(6, Math.min(36, osz + (e2.clientX - startX) / s * 0.3)); const tb = box.querySelector('table'); if (tb) tb.style.fontSize = (o.size * s) + 'px'; }
     if (kindCls === 'text' || kindCls === 'date') o.size = Math.max(8, Math.min(72, osz + (e2.clientX - startX) / s * 0.6));
     box.style.width = (o.w * s) + 'px';
     if (kindCls === 'text' || kindCls === 'date') box.style.fontSize = (o.size * s) + 'px';
   };
   const onUp = () => { window.removeEventListener('pointermove', onMove); window.removeEventListener('pointerup', onUp); smartPdfOverlayStore(page, o); };
   window.addEventListener('pointermove', onMove); window.addEventListener('pointerup', onUp);
 });
 delbtn.addEventListener('click', (ev) => { ev.preventDefault(); ev.stopPropagation(); try { if (kindCls === 'table') box.querySelectorAll('td[contenteditable]').forEach((td) => td.blur()); } catch (e2) {} smartPdfOverlayRemove(page, o.id); smartPdfRenderOverlays(); });
 return box;
}
// Re-render all overlay layers across pages (from the store; replaces stale DOM.)
function smartPdfRenderOverlays() {
  if (typeof document === 'undefined' || !smartImportOverlays) return;
  const holder = smartImportEl('smartPdfEditor');
 if (!holder) return;
 holder.querySelectorAll('.smart-pdf-overlay-layer').forEach((l) => l.remove());
 const pages = Object.keys(smartImportOverlays);
 for (const pk of pages) {
   const list = smartImportOverlays[pk] || []; if (!list.length) continue;
    const page = parseInt(pk, 10);
   const prev = smartImportCurrentPage; smartImportCurrentPage = page;
   list.forEach((o) => {
     if (!o) return;
     const t = smartPdfOverlayTarget(); if (!t || !t.wrap) return;
     t.wrap.classList.add('has-overlay');
     switch (o.type) {
       case 'text': smartPdfOverlayBox(page, o, 'text', smartPdfEscHtml(o.text || ''), true); break;
       case 'date': smartPdfOverlayBox(page, o, 'date', smartPdfEscHtml(o.text || ''), true); break;
       case 'image': case 'logo': case 'signature': case 'stamp':
         smartPdfOverlayBox(page, o, 'image', '<img src="' + smartPdfEscAttr(o.dataUrl || '') + '" alt="">', false); break;
       case 'table': smartPdfOverlayBox(page, o, 'table', smartPdfOverlayTableHtmlP23(o.rows || [['', ''], ['', '']], o), true); break;
     }
   });
...
async function smartPdfAddItem(type) {
  ...
  switch (type) {
    ...
    case 'image':
      ...
      o.dataUrl = await smartPdfUrlToDataUrl(url2); o.w = baseW * 0.3; break;
    case 'logo':
    ...
    case 'table':
            o.rows = smartPdfTableNormalize([['Item', 'Quantity', 'Price'], ['Item A', '10', '50'], ['Item B', '5', '25']]).map((r) => r.map((c) => ({ text: c.text, align: c.align, bg: c.bg, color: c.color, bold: c.bold, italic: c.italic }))); o.w = baseW * 0.5; o.align = o.align || 'left'; o.bg = o.bg || ''; o.color = o.color || ''; o.borderColor = o.borderColor || 'rgba(15, 23, 42, 0.45)'; o.borderWidth = 1; o.hasBorder = true; break; // PART 23
...
async function smartImportTrueEditBytes(pdfLib, pakoObj, originalBytes, editsByPage, pages, colorsByPage, overlaysByPage) {
        ...
        for (const o of list) {
          ...
          try {
            if (o.type === 'text' || o.type === 'date') {
              ...
              // PART 23 — structured table render: the SAME cell-object model the
              // editor uses (text/align/bg/color/bold/italic + border on/off and
              // border color), drawn as REAL pdf-lib text + lines (never an image).
              const rows = smartPdfTableNormalize(o.rows);
              const size = Math.max(6, (o.size || 12) * f);
              const W = (o.w || 200) * f;
              const cols = (rows[0] || []).length || 1;
              const colW = W / cols;
              const rowH = (o.rowH ? o.rowH * f : size * 1.55);
              const bw = (typeof o.borderWidth === 'number' && o.borderWidth > 0) ? Math.min(4, o.borderWidth * f) : 1;
              const bc23 = smartPdfPdfColorP23(o.borderColor || 'rgba(15, 23, 42, 0.45)');
              let helvB = null, helvI = null, helvBI = null;
              const pickFont23 = async (cell) => {
                try {
                  if (cell.bold && cell.italic) { if (!helvBI) helvBI = await doc.embedFont(pdfLib.StandardFonts.HelveticaBoldOblique); return helvBI; }
                  if (cell.bold) { if (!helvB) helvB = await doc.embedFont(pdfLib.StandardFonts.HelveticaBold); return helvB; }
                  if (cell.italic) { if (!helvI) helvI = await doc.embedFont(pdfLib.StandardFonts.HelveticaOblique); return helvI; }
                } catch (e5) {}
                return helv;
              };
              const rowHs23 = rows.map((row) => {
                if (o.rowH) return rowH;
                let lines = 1;
                for (const cell of row) {
                  const ct = String((cell && cell.text) || '');
                  if (!ct || !helv) continue;
                  let tw = 0; try { tw = helv.widthOfTextAtSize(ct, size); } catch (e6) { tw = ct.length * size * 0.5; }
                  const usable = Math.max(10, colW - 6 * f);
                  lines = Math.max(lines, Math.ceil(tw / usable));
                }
                return Math.max(rowH, size * 1.3 * lines);
              });
              let yy = H - yTop;
              for (let r = 0; r < rows.length; r++) {
                ...
                for (let c = 0; c < row.length; c++) {
                  const cell = row[c];
                  const ct = String((cell && cell.text) || '');
                  if (cell.bg && cell.bg.toUpperCase() !== '#FFFFFF') {
                    const fill23 = smartPdfPdfColorP23(cell.bg);
                    try { pageObj.drawRectangle({ x: xx, y: yy - rh, width: colW, height: rh, color: pdfLib.rgb(fill23.r, fill23.g, fill23.b) }); } catch (e3) {}
                  }
                  if (ct && helv) {
                    let txtW = 0; try { txtW = helv.widthOfTextAtSize(ct, size); } catch (e4) { txtW = ct.length * size * 0.5; }
                    const pad = 3 * f;
                    let tx = xx + pad;
                    if (cell.align === 'center') tx = xx + Math.max(pad, (colW - Math.min(txtW, colW - pad * 2)) / 2);
                    else if (cell.align === 'right') tx = xx + colW - pad - Math.min(txtW, colW - pad * 2);
                    const fnt = await pickFont23(cell);
              ...
              if (o.hasBorder !== false) {
                const bottom23 = yy + rowHs23.reduce((s5, z) => s5 + z, 0);
                for (let r = 0; r <= rows.length; r++) {
                  let hr5 = yy; for (let z = 0; z < r; z++) hr5 += rowHs23[z];
                  pageObj.drawLine({ start: { x: x, y: hr5 }, end: { x: x + W, y: hr5 }, thickness: bw, color: pdfLib.rgb(bc23.r, bc23.g, bc23.b) });
                }
                for (let c = 0; c <= cols; c++) pageObj.drawLine({ start: { x: x + c * colW, y: yy }, end: { x: x + c * colW, y: bottom23 }, thickness: bw, color: pdfLib.rgb(bc23.r, bc23.g, bc23.b) });
              }
...
function wireEvents() {
  ...
  // PART 19 — Existing PDF → PDF Editor entry from the PDF Workspace.
  // The "Open PDF" card opens the existing stable Smart Documents import flow
  // (smartImportOpen): device file picker → PDF validation → pdf.js parse →
  // the in-place TRUE-PDF editor (PART 5e) whose Save/Export rewrites the
  // ORIGINAL content streams via pdf-lib (real text, no flattening). No new
  // editor/viewer/storage: this reuses the existing flow exactly.
  const pdfOpenCard = document.getElementById('pdfOpenCard');
  if (pdfOpenCard) {
    pdfOpenCard.addEventListener('click', () => {
      closePdfReportsWorkspace();
      openSmartDocs();
      smartImportOpen();
    });
  ...
  // PART 22 - Add menu (Text/Image/Logo/Signature/Stamp/Date/Table) in the PDF editor.
  // Same open/close popover pattern as the existing Text Color menu; every item routes
  // through the existing smartPdfAddItem(type) overlay model (PDF editor only).
  const smartPdfAddBtn = document.getElementById('smartPdfAddBtn');
  const smartPdfAddWrap = document.getElementById('smartPdfAddWrap');
  const smartPdfAddMenu = document.getElementById('smartPdfAddMenu');
  const smartPdfAddClose = () => {
    if (smartPdfAddMenu) smartPdfAddMenu.setAttribute('hidden', '');
    if (smartPdfAddBtn) smartPdfAddBtn.setAttribute('aria-expanded', 'false');
  };
  if (smartPdfAddBtn && smartPdfAddWrap && smartPdfAddMenu) {
    smartPdfAddBtn.addEventListener('click', (ev) => {
      ev.stopPropagation();
      const willOpen = smartPdfAddMenu.hasAttribute('hidden');
      if (willOpen) smartPdfAddMenu.removeAttribute('hidden'); else smartPdfAddMenu.setAttribute('hidden', '');
      smartPdfAddBtn.setAttribute('aria-expanded', willOpen ? 'true' : 'false');
    });
    smartPdfAddMenu.addEventListener('click', (ev) => {
      const it = ev.target && ev.target.closest ? ev.target.closest('.smart-pdf-add-item') : null;
      if (!it) return;
      ev.stopPropagation();
      const type = it.getAttribute('data-add') || '';
      try { smartPdfAddItem(type); } catch (e) {}
      smartPdfAddClose();
    });
    smartPdfAddMenu.addEventListener('keydown', (ev) => {
      if (ev.key === 'Escape') { smartPdfAddClose(); smartPdfAddBtn.focus(); }
    });
    document.addEventListener('click', (ev) => {
      if (!smartPdfAddWrap.contains(ev.target)) smartPdfAddClose();
    });
  }
  if (typeof window !== 'undefined' && window.__smartImport) {
    window.__smartImport.addItem = function (t) { return smartPdfAddItem(t); };
    window.__smartImport.overlays = function () { return smartImportOverlays; };
    window.__smartImport.currentPage = function () { return smartImportCurrentPage; };
    window.__smartImport.setCurrentPage = function (pg) { smartImportCurrentPage = Number(pg) || 0; };
    // PART 23 — structured-table debug/test seam (reuses the SAME store + renderer; no second engine).
    window.__smartPdfTables = {
      rebuild: function (rowsN, colsN) {
        let target = null, tpage = -1;
        for (const pk in (smartImportOverlays || {})) {
          (Array.isArray(smartImportOverlays[pk]) ? smartImportOverlays[pk] : []).forEach((o) => { if (o && o.type === 'table') { target = o; tpage = Number(pk); } });
        }
        if (!target) return false;
        const rows = [], rN = Math.max(1, rowsN | 0), cN = Math.max(1, colsN | 0);
        for (let r = 0; r < rN; r++) { const row = []; for (let c = 0; c < cN; c++) row.push(smartPdfCellObj('')); rows.push(row); }
        smartPdfTableApplyP23(tpage, target.id, (oo) => { oo.rows = rows; });
        return true;
      },
      applyModel: function (id, patch) {
        for (const pk in (smartImportOverlays || {})) {
          const list = Array.isArray(smartImportOverlays[pk]) ? smartImportOverlays[pk] : [];
          const o = list.find((x) => x && x.type === 'table' && x.id === id);
          if (o) { smartPdfTableApplyP23(Number(pk), id, (oo) => { Object.keys(patch || {}).forEach((k) => { oo[k] = patch[k]; }); }); return true; }
        }
        return false;
      },
    };
