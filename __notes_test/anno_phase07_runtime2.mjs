...
const LOG = 'C:\\Temp\\p07run.log';
const results = [];
try { fs.rmSync(LOG, { force: true }); } catch (e) {}
...
// Watchdog: force-exit if the flow ever hangs (await keeps event loop alive, so
// this timer still runs even when a CDP call never resolves).
const watchdog = setTimeout(() => {
  log('WATCHDOG: flow hung — dumping partial results and exiting');
  const pass = results.filter((r) => r.ok).length;
  console.log(`\n==== PHASE 07 (PARTIAL): ${pass} passed, ${results.length - pass} failed ====`);
  process.exit(2);
}, 90000);
watchdog.unref();
let browser;
try {
  log('STEP 0: launch + open note + open preview');
  ...
  const layerRect = async () => page.evaluate(() => { let r = document.getElementById('notePdfAnnoLayer').getBoundingClientRect(); if (!r.width) r = document.getElementById('notePdfPreviewCanvas').getBoundingClientRect(); return { x: r.x, y: r.y, w: r.width, h: r.height }; });
  const noteBoxes = (f) => page.evaluate((cb) => Array.from(document.querySelectorAll('#notePdfAnnoTextBoxes .note-pdf-anno-textbox')).filter((b) => !cb || b.className.includes(cb)).map((b) => ({ cls: b.className, text: b.textContent || '', dir: b.dir, r: (() => { const r = b.getBoundingClientRect(); return { x: r.x, y: r.y, w: r.width, h: r.height }; })() })), f);
  const chipState = () => page.evaluate(() => { const c = document.getElementById('notePdfAnnoDeleteChip'); return c ? { hidden: c.hidden } : null; });

  // A) Note tool + typing (Arabic/English/digits).
  log('STEP A: note tool create + type');
  check('New 📝 Note inline button present', await page.evaluate(() => !!document.getElementById('notePdfAnnoNoteBtn')));
  await page.evaluate(() => { document.getElementById('notePdfAnnoNoteBtn').click(); });
  await sleep(120);
  let lr = await layerRect();
  await page.mouse.click(lr.x + lr.w * 0.35, lr.y + lr.h * 0.4);
  await sleep(400);
  let bs = await noteBoxes('note-pdf-anno-notebox');
  check('Note tool creates a 📝 note box on the page', bs.length === 1, JSON.stringify(bs.map((b) => b.cls)));
  await page.keyboard.type('مرحبا Hello 123');
  await sleep(300);
  bs = await noteBoxes('note-pdf-anno-notebox');
  const txt = bs[0] ? bs[0].text : '';
  const hasArabic = /[\u0600-\u06FF]/.test(txt);
  const hasDigit = /\d/.test(txt);
  const hasLatin = /[A-Za-z]/.test(txt);
  check('Note supports Arabic + English + numbers (mixed)', bs.length === 1 && hasArabic && hasDigit && hasLatin, JSON.stringify({ txt, hasArabic, hasDigit, hasLatin, dir: bs[0] ? bs[0].dir : null }));
// B) Selection via edit + delete chip.
  log('STEP B: selection + chip');
  await page.evaluate(() => { document.getElementById('notePdfAnnoEditBtn').click(); });
  await sleep(150);
  bs = await noteBoxes('note-pdf-anno-notebox');
  if (bs[0]) { await page.mouse.click(bs[0].r.x + bs[0].r.w / 2, bs[0].r.y + bs[0].r.h / 2); await sleep(300); }
  const chipv = await chipState();
  check('Clicking the note selects it and reveals the delete chip (selection model)', chipv !== null && !chipv.hidden, JSON.stringify(chipv));

  // C) Move via drag.
  log('STEP C: move note');
  bs = await noteBoxes('note-pdf-anno-notebox');
  const before = bs[0] ? bs[0].r : null;
  if (before) {
    await page.mouse.move(before.x + before.w / 2, before.y + before.h / 2);
    await page.mouse.down();
    await page.mouse.move(before.x + before.w / 2 + 45, before.y + before.h / 2 + 28, { steps: 6 });
    await page.mouse.up();
    await sleep(300);
  }
  bs = await noteBoxes('note-pdf-anno-notebox');
  ...
  // D) Delete chip removes the annotation.
  log('STEP D: delete chip');
  await page.evaluate(() => { document.getElementById('notePdfAnnoEditBtn').click(); });
  await sleep(150);
  bs = await noteBoxes('note-pdf-anno-notebox');
  if (bs[0]) { await page.mouse.click(bs[0].r.x + bs[0].r.w / 2, bs[0].r.y + bs[0].r.h / 2); await sleep(250); }
  ...
  bs = await noteBoxes('note-pdf-anno-notebox');
  check('Delete chip removes the selected annotation', bs.length === 0, JSON.stringify(bs.map((b) => b.cls)));

  // E) Multi-page: shape on page 2, page 1 stays clean.
  log('STEP E: multi-page');
  await page.evaluate(() => { document.getElementById('notePdfNextPageBtn').click(); });
  await page.waitForFunction(() => (document.getElementById('notePdfPageIndicator') || {}).textContent === '2 / 2', { timeout: 12000 });
  await sleep(300);
  lr = await layerRect();
  await page.evaluate(() => { document.getElementById('notePdfAnnoRectBtn').click(); });
  await sleep(150);
  ...
  await page.evaluate(() => { document.getElementById('notePdfPrevPageBtn').click(); });
  await page.waitForFunction(() => (document.getElementById('notePdfPageIndicator') || {}).textContent === '1 / 2', { timeout: 12000 });
  ...
  check('Annotations stay associated with their own page (page 1 clean)', bs.length === 0 && !p1Canvas, JSON.stringify({ boxes: bs.length, painted: p1Canvas }));
// F) Zoom alignment.
  log('STEP F: zoom');
  await page.evaluate(() => { document.getElementById('notePdfNextPageBtn').click(); });
  await page.waitForFunction(() => (document.getElementById('notePdfPageIndicator') || {}).textContent === '2 / 2', { timeout: 12000 });
  await sleep(300);
      // Wait for the overlay to re-align with the canvas after a zoom/page change.
  // renderNotePdfPreviewPage is async (pdf.js getPage + render + rAF re-project);
  // a fixed sleep is too fragile across headless machines, so poll until the
  // layer bounding rect matches the canvas bounding rect (within 3px).
  const waitForAligned = () => page.waitForFunction(() => {
    ...
    return Math.abs(Math.round(layer.width) - Math.round(cv.width)) <= 3 &&
      Math.abs(Math.round(layer.height) - Math.round(cv.height)) <= 3;
  }, { timeout: 8000 }).catch(() => { /* fall through; measurement follows */ });
  const aligned = async () => page.evaluate(() => {
    const layer = document.getElementById('notePdfAnnoLayer').getBoundingClientRect();
    const cv = document.getElementById('notePdfPreviewCanvas').getBoundingClientRect();
    return { close: Math.abs(Math.round(layer.width) - Math.round(cv.width)) <= 3 && Math.abs(Math.round(layer.height) - Math.round(cv.height)) <= 3, lw: Math.round(layer.width), cw: Math.round(cv.width) };
  });
  ...
  // G) Lifecycle cleanup.
  log('STEP G: close/reopen cleanup');
  ...
  log(`==== PHASE 07 RUNTIME: ${pass} passed, ${fail} failed, ${results.length} total ====`);
