...
function onSmartScanAccept() {
  ...
  smartScanRenderStructure();
  state.smartScanResult = { text: text, status: 'accepted', structure: smartScanStructure };
  const info = scanEl('scanAcceptInfo');
...
const SMART_STRUCT_FIELD_LABELS = [
  ...
  'الاسم', 'التاريخ', 'الهاتف', 'البريد', 'العنوان', 'الشركة', 'الاجمالي',
...
function smartScanAnalyzeStructure(text) {
  ...
  const fields = [];
  const fieldLineRe = /^([^\s:][^:\n]{0,31})\s*:\s*(.*)$/;
  const fieldUnderRe = /^([^\s_][^_\n]{0,31})[ \t]+_{2,}\s*(.*)$/;
  nonEmpty.forEach((ln) => {
    if (inTable(ln.idx)) return;
    const mm = fieldLineRe.exec(ln.text) || fieldUnderRe.exec(ln.text);
    if (!mm) return;
    ...
    if (!known) return;
    const value = (mm[2] || '').trim().replace(/^[_\s]+/, '').replace(/[_\s]+$/, '');
    fields.push({ type: 'field', label: label, value: value, hasValue: value !== '', line: ln.idx });
  });
