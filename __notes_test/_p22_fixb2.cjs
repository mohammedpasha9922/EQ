const fs = require('fs');
const hPath = 'd:/Programs EQ7/EQ/index.html';
const cPath = 'd:/Programs EQ7/EQ/styles.css';
let h = fs.readFileSync(hPath, 'utf8');
const anchor = '<span class="smart-pdf-toolbar-spacer" aria-hidden="true"></span>';
if (h.split(anchor).length !== 2) throw new Error('html anchor');
const MENU = [
...
s = s.replace(wireAnchor, WIRE);
if (s !== b) fs.writeFileSync(p, s);
console.log('FIXB2 OK CHANGED=' + (s !== b));