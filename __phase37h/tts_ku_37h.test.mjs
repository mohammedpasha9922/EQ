// PHASE 37H — real-browser Kurdish TTS verification (test-only artifact).
...
// ---------- Kurdish: Calculator speaker (multiple results) ----------
await boot();
await installCapture();
await setLang('ku');
await clickCalcSpeaker();
...
check('Kurdish Calculator speaker produced TTS requests (multiple results)', c.length >= 2, JSON.stringify(c));
check('Kurdish Calculator TTS never uses English locale', c.length > 0 && c.every((x) => !/^en(-|_|$)/i.test(x.lang || 'en')), JSON.stringify(c));
check('Kurdish Calculator TTS requests ku/ckb/kmr locale', c.length > 0 && c.every((x) => /^(ku|ckb|kmr)/i.test(x.lang || '')), JSON.stringify(c));
check('Kurdish: spoken text stays Kurdish numeric text (no English number words)', c.length > 0 && !/\b(twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred)\b/i.test(c.map((x) => x.text).join(' ')), `text=${c.map((x) => x.text).join('|')}`);
check('Kurdish Calculator TTS never attaches an English voice', c.length > 0 && c.every((x) => x.voiceName == null || !/^en(-|_|$)/i.test(x.voiceLang || '')), JSON.stringify(c));

// ---------- Kurdish: History speaker (uses entry created above) ----------
...
check('Kurdish History TTS never attaches an English voice', h.length > 0 && (h[0]?.voiceName == null || !/^en(-|_|$)/i.test(h[0]?.voiceLang || '')), JSON.stringify(h[0] || {}));
// Injected selection proof: same pick-order as production must choose the Kurdish entry.
const injPick = await page.evaluate(() => {
  const vs = [{ name: 'Fake Kurdish Sorani', lang: 'ckb-IQ' }, { name: 'Google US English', lang: 'en-US' }];
  return (vs.find((v) => /^ku([-_]|$)/i.test(v.lang || ''))
    || vs.find((v) => /^(ckb|kmr)([-_]|$)/i.test(v.lang || ''))
    || vs.find((v) => /kurdish|sorani|kurmanji|kurmanc|kurd/i.test(v.name || '')));
});
check('Injected ckb voice is selected as Kurdish (code-level selection logic)', !!injPick && /^(ckb|kmr)(-|_|$)/i.test(injPick.lang || ''), JSON.stringify(injPick || {}));
...
if (kuVoices.length === 0) {
  console.log('Kurdish voice unavailable in test environment');
...
console.log('PHASE 37H TEST DONE');
