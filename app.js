...
const iosInstallStep3 = typeof document !== 'undefined' ? document.getElementById('iosInstallStep3') : null;
...
const translations = {
  ...
  pdfTplBlank: 'Blank', pdfTplReport: 'Report', pdfTplInvoice: 'Invoice', pdfTplReceipt: 'Receipt', pdfTplContract: 'Contract', pdfTplCv: 'CV', pdfTplBusiness: 'Business Report', pdfTplEngineering: 'Engineering Report', pdfTplLetter: 'Letter',
    ...
    smartPdfShareBtn: 'Share PDF',
    smartPdfReplaceBtn: 'Open another PDF',
    ...
    installModalStep3: 'Step 3: Confirm adding it to the Home Screen.',
  ...
  pdfTplBlank: 'En blanco', pdfTplReport: 'Informe', pdfTplInvoice: 'Factura', pdfTplReceipt: 'Recibo', pdfTplContract: 'Contrato', pdfTplCv: 'CV', pdfTplBusiness: 'Informe empresarial', pdfTplEngineering: 'Informe de ingeniería', pdfTplLetter: 'Carta',
    ...
    smartPdfShareBtn: 'Compartir PDF',
    smartPdfReplaceBtn: 'Abrir otro PDF',
    ...
    installModalStep3: 'Paso 3: Confirma la adición a la pantalla de inicio.',
  ...
  pdfTplBlank: 'فارغ', pdfTplReport: 'تقرير', pdfTplInvoice: 'فاتورة', pdfTplReceipt: 'إيصال', pdfTplContract: 'عقد', pdfTplCv: 'سيرة ذاتية', pdfTplBusiness: 'تقرير أعمال', pdfTplEngineering: 'تقرير هندسي', pdfTplLetter: 'رسالة',
    ...
    smartPdfShareBtn: 'مشاركة PDF',
    smartPdfReplaceBtn: 'فتح PDF آخر',
    ...
    installModalStep3: 'الخطوة 3: أكد الإضافة إلى الشاشة الرئيسية.',
  ...
  pdfTplBlank: 'Vierge', pdfTplReport: 'Rapport', pdfTplInvoice: 'Facture', pdfTplReceipt: 'Reçu', pdfTplContract: 'Contrat', pdfTplCv: 'CV', pdfTplBusiness: "Rapport d'affaires", pdfTplEngineering: "Rapport d'ingénierie", pdfTplLetter: 'Lettre',
    ...
    smartPdfShareBtn: 'Partager le PDF',
    smartPdfReplaceBtn: 'Ouvrir un autre PDF',
    ...
    installModalStep3: 'Étape 3 : Confirmez l\'ajout à l\'écran d\'accueil.',
  ...
  pdfTplBlank: 'Пустой', pdfTplReport: 'Отчёт', pdfTplInvoice: 'Счёт', pdfTplReceipt: 'Квитанция', pdfTplContract: 'Договор', pdfTplCv: 'Резюме', pdfTplBusiness: 'Деловой отчёт', pdfTplEngineering: 'Инженерный отчёт', pdfTplLetter: 'Письмо',
    ...
    smartPdfShareBtn: 'Поделиться PDF',
    smartPdfReplaceBtn: 'Открыть другой PDF',
    ...
    installModalStep3: 'Шаг 3: Подтвердите добавление на главный экран.',
  ...
  pdfTplBlank: 'Leer', pdfTplReport: 'Bericht', pdfTplInvoice: 'Rechnung', pdfTplReceipt: 'Quittung', pdfTplContract: 'Vertrag', pdfTplCv: 'Lebenslauf', pdfTplBusiness: 'Geschäftsbericht', pdfTplEngineering: 'Ingenieurbericht', pdfTplLetter: 'Brief',
    ...
    smartPdfShareBtn: 'PDF teilen',
    smartPdfReplaceBtn: 'Anderes PDF öffnen',
    ...
    installModalStep3: 'Schritt 3: Bestätigen Sie das Hinzufügen zum Home-Bildschirm.',
  ...
  pdfTplBlank: 'Boş', pdfTplReport: 'Rapor', pdfTplInvoice: 'Fatura', pdfTplReceipt: 'Makbuz', pdfTplContract: 'Sözleşme', pdfTplCv: 'Özgeçmiş', pdfTplBusiness: 'İş raporu', pdfTplEngineering: 'Mühendislik raporu', pdfTplLetter: 'Mektup',
    ...
    smartPdfShareBtn: 'PDF paylaş',
    smartPdfReplaceBtn: 'Başka PDF aç',
    ...
    installModalStep3: 'Adım 3: Ana ekrana eklemeyi onaylayın.',
  ...
  ku: {
    ...
    "smartPdfShareBtn": "هاوبەشکردنی PDF",
    "smartPdfReplaceBtn": "کردنەوەی PDFێکی تر",
    ...
    "installModalStep3": "هەنگاوی 3: زیادکردن بۆ شاشەی سەرەکی پشتڕاست بکەرەوە.",
...
function updateInstallModalContent() {
  ...
  if (typeof iosInstallStep3 !== 'undefined' && iosInstallStep3) iosInstallStep3.textContent = t.installModalStep3 || '';
}
...
function closePdfReportsWorkspace() {
  ...
  // PDF Reports loaded state reset: back to the empty/upload state.
  try {
    const modalEl = ws && ws.querySelector('.pdf-reports-workspace');
    if (modalEl) modalEl.classList.remove('pdf-reports-loaded');
    const viewerEl = document.getElementById('pdfV1Viewer');
    const wrapEl = document.getElementById('pdfV1ViewerWrap');
    if (viewerEl) viewerEl.setAttribute('src', '');
...
let smartPdfV3File = null; // current PDF File (kept so Share/Export can rebuild it)
let smartPdfV3Bytes = null; // pristine bytes of the current PDF (for pdf-lib reload)
let smartPdfV3ShareBusy = false;
...
function smartPdfV3ShowMode(mode) {
  ...
  // Top Share/Replace icon buttons exist only alongside the viewer.
  const shareBtn = smartPdfV3El('smartPdfShareBtn');
  const replaceBtn = smartPdfV3El('smartPdfReplaceBtn');
  if (shareBtn) shareBtn.hidden = !showViewer;
  if (replaceBtn) replaceBtn.hidden = !showViewer;
  try { smartPdfV3SyncTopLabels(); } catch (e) { /* labels stay in English */ }
}

// Keep icon-only buttons accessible: mirror the localized title
// (maintained by the shared updateTexts via data-i18n-title) into aria-label.
function smartPdfV3SyncTopLabels() {
  const t = smartPdfV3T();
  const shareBtn = smartPdfV3El('smartPdfShareBtn');
  const replaceBtn = smartPdfV3El('smartPdfReplaceBtn');
  if (shareBtn && t.smartPdfShareBtn) {
    shareBtn.setAttribute('title', t.smartPdfShareBtn);
    shareBtn.setAttribute('aria-label', t.smartPdfShareBtn);
  }
  if (replaceBtn && t.smartPdfReplaceBtn) {
    replaceBtn.setAttribute('title', t.smartPdfReplaceBtn);
    replaceBtn.setAttribute('aria-label', t.smartPdfReplaceBtn);
  }
}
...
async function smartPdfV3OpenFile(file) {
  ...
  try {
    ...
    // Pristine copy BEFORE pdf.js (getDocument detaches its buffer).
    try { smartPdfV3Bytes = raw.slice(0); } catch (e) { smartPdfV3Bytes = raw; }
    smartPdfV3File = file;
    const data = raw;
...
// DIRECTION + SINGLE-LINE OVERFLOW FIX (Smart PDF text layer ONLY — no
// renderer, canvas, font, DPR, zoom, scroll or layout change; no scale,
// no letter-spacing, no font-size change).
// Root cause of the Arabic/RTL bug: the previous box logic clamped a longer
// replacement to the free gap before its same-line neighbour
// (maxWidth = gap) AND forced `white-space: pre-wrap` + `break-word` +
// `overflow: hidden` inside that narrow box. A 4-char Arabic word replaced
// by a 9-char word no longer fits in the gap-width box, so the browser
// wrapped it vertically (letter under letter, space-triggered wraps,
// glyphs painting over the next word). The clamp re-measured the already
// narrowed box on every keystroke, so the box could never grow back.
// Fix: the edited run keeps its original line anchor (LTR pins left, RTL
// pins right) and stays strictly single-line (`white-space: pre`,
// `overflow: visible`, no neighbour maxWidth), so extra characters continue
// horizontally in the run's own direction with natural Arabic shaping,
// correct caret movement and natural spaces. Neighbour spans, the page
// flow and every other feature are untouched (no full reflow by design:
// a replacement longer than its gap visibly continues over the gap area
...
// 2. A longer replacement stays on its own single line anchored at its
//    original edge (LTR: left, RTL: right) and overflows horizontally in its
//    own direction instead of wrapping vertically or painting neighbour text.
...
function smartPdfV3ClampEditBox(span) {
  ...
  try {
    // Keep the run strictly single-line: extra characters (longer Arabic
    // word, text after a space, mixed RTL/LTR runs) continue horizontally
    // in the run's own direction instead of wrapping letter-under-letter.
    // No neighbour gap clamp and no maxWidth: clamping to the neighbour
    // gap is exactly what squeezed Arabic replacements into a narrow box
    // and produced the vertical stacking. Long replacements intentionally
    // overflow over the gap area (no full page reflow by design).
    if (dir === 'ltr') {
      if (span._smartPdfBoxL != null) span.style.left = span._smartPdfBoxL + 'px';
      span.style.right = 'auto';
    } else {
      const anchorR = (span._smartPdfBoxR != null)
        ? span._smartPdfBoxR
    ...
    span.style.whiteSpace = 'pre'; // single line: spaces stay natural, never wrap
    span.style.overflowWrap = 'normal';
    span.style.wordBreak = 'normal';
    span.style.overflow = 'visible';
    span.style.height = 'auto';
    if (span.style.lineHeight) span.style.minHeight = span.style.lineHeight;
    smartPdfV3NudgeNeighbour(span, dir, layer, layerW);
  } catch (e) { /* bounding is best-effort — editing itself unaffected */ }
}

// LONGER-REPLACEMENT OVERLAP FIX (Smart PDF text layer ONLY — scoped to the
// edited run + at most ONE same-line neighbour).
// A longer replacement keeps its own single-line width (see above), so its
// overflow side would otherwise paint over the next word. This shifts ONLY
// the nearest same-line neighbour on the overflow side (LTR: the run right
// after; RTL: the run right before in screen coordinates) by the exact
// overflow amount via CSS transform — no page reflow, no word-wrap change,
// no reordering, no layout change. The shift is reverted with the edit.
function smartPdfV3NudgeNeighbour(span, dir, layer, layerW) {
  try {
    const prev = span._smartPdfNudged;
    if (prev && prev.isConnected) prev.style.transform = prev._smartPdfNudgeOrig || '';
    span._smartPdfNudged = null; span._smartPdfOverflowPx = 0;
    if (!span || !layer) return;
    const r = span.getBoundingClientRect();
    if (!r.width) return;
    const origR = (span._smartPdfBoxL != null)
      ? { left: span._smartPdfBoxL, right: span._smartPdfBoxR }
      : null;
    let overflow = 0;
    if (dir === 'ltr' && origR) overflow = (span.offsetLeft + span.offsetWidth) - origR.right;
    else if (dir === 'rtl' && origR) overflow = origR.left - span.offsetLeft;
    else return;
    if (!(overflow > 2)) return; // fits: no neighbour touch at all
    const midY = r.top + r.height / 2;
    let best = null, bestGap = Infinity;
    const kids = layer.children;
    for (let i = 0; i < kids.length; i++) {
      const k = kids[i];
      if (k === span || k.nodeName !== 'SPAN') continue;
      const kr = k.getBoundingClientRect();
      if (!kr.width) continue;
      if (midY < kr.top - 2 || midY > kr.bottom + 2) continue; // other line: skip
      if (dir === 'ltr') {
        const gap = kr.left - r.left;
        if (kr.left >= (r.left + r.width * 0.4) && gap < bestGap) { bestGap = gap; best = k; }
      } else {
        const gap = r.left - kr.left;
        if ((kr.left + kr.width) <= (r.left + r.width * 0.6) && gap < bestGap) { bestGap = gap; best = k; }
      }
    }
    if (!best) return;
    const shift = Math.ceil(overflow) + 2;
    if (best._smartPdfNudgeOrig === undefined) best._smartPdfNudgeOrig = best.style.transform || '';
    best.style.transform = 'translateX(' + (dir === 'ltr' ? shift : -shift) + 'px)';
    span._smartPdfNudged = best;
    span._smartPdfOverflowPx = shift;
  } catch (e) { /* neighbour untouched — editing itself unaffected */ }
}
...
function smartPdfV3RestoreEditBox(span) {
  if (!span) return;
  try {
    const nb = span._smartPdfNudged;
    if (nb && nb.isConnected) nb.style.transform = nb._smartPdfNudgeOrig || '';
  } catch (e) { /* ignore */ }
  try {
    ...
    if (o) {
      ...
      span.style.overflowWrap = o.overflowWrap; span.style.wordBreak = o.wordBreak;
      span.style.overflow = o.overflow;
...
function smartPdfV3StartEdit(span, clientX, clientY) {
  ...
  if (index === null || index < 0 || index > textLen) {
    // MULTILINGUAL CARET FIX (click accuracy for Arabic/Kurdish/RTL + mixed):
    // caretRangeFromPoint on a transparent zero-contrast span can miss on some
    // browsers, and the old midpoint fallback jumped to far/near end. Resolve
    // per-character hit rects instead (screen-space), then pick the nearest
    // boundary — with a small padded hit area so dense/shaped glyphs and
    // right-edge RTL clicks still map to the correct boundary. Direction only
    // decides ties, never mirrors the choice.
    try {
      const r0 = span.getBoundingClientRect();
      const doc0 = span.ownerDocument;
      const rng0 = doc0.createRange();
      rng0.selectNodeContents(span);
      const rects = rng0.getClientRects ? rng0.getClientRects() : [];
      let bestI = -1, bestD = Infinity, bestSide = 0;
      for (let ci = 0; ci <= textLen; ci++) {
        let bx = null;
        try {
          const rr = doc0.createRange();
          const tn = span.firstChild && span.firstChild.nodeType === 3 ? span.firstChild : null;
          if (!tn) break;
          rr.setStart(tn, Math.max(0, Math.min(ci, tn.data.length)));
          rr.collapse(true);
          const q = rr.getClientRects ? rr.getClientRects()[0] : null;
          if (q) bx = q.left;
        } catch (e2) { bx = null; }
        if (bx === null || bx === undefined) {
          if (r0.width) bx = (span.getAttribute('dir') === 'rtl')
            ? (r0.right - (ci / Math.max(1, textLen)) * r0.width)
            : (r0.left + (ci / Math.max(1, textLen)) * r0.width);
          else bx = r0.left;
        }
        // Pad: clicks slightly below/above shaped glyphs still hit the row.
        const d = Math.abs(clientX - bx) + Math.max(0, Math.abs(clientY - (r0.top + r0.height / 2)) - r0.height) * 0.25;
        if (d < bestD - 0.5) { bestD = d; bestI = ci; bestSide = (clientX >= bx) ? 1 : -1; }
      }
      if (bestI >= 0) index = bestI;
    } catch (e3) { /* fall through to midpoint below */ }
  }
  if (index === null || index < 0 || index > textLen) {
    const r = span.getBoundingClientRect();
  ...
  // replacements continue single-line in their own direction.
  if (!span._smartPdfBoxOrig) {
    span._smartPdfBoxOrig = {
      ...
      wordBreak: span.style.wordBreak,
...
// Share = CURRENT (edited) PDF via OS sheet; Replace = open picker.
// Scoped strictly to #smartPdfWorkspace; every other feature untouched.
function smartPdfV3EditedSpans() {
  const pagesEl = smartPdfV3El('smartPdfViewerPages');
  if (!pagesEl) return [];
  const out = [];
  const spans = pagesEl.querySelectorAll('.smart-pdf-text-edited');
  for (let i = 0; i < spans.length; i++) {
    const span = spans[i];
    if (!span || !span.isConnected) continue;
    const txt = span.textContent || '';
    if (!txt) continue;
    const pageEl = span.closest ? span.closest('.smart-pdf-viewer-page') : null;
    const canvas = pageEl ? pageEl.querySelector('canvas') : null;
    if (!pageEl || !canvas) continue;
    const n = parseInt(pageEl.getAttribute('data-page') || '0', 10);
    if (!n || n < 1) continue;
    const cw = canvas.clientWidth, ch = canvas.clientHeight;
    if (!cw || !ch) continue;
    const sx = span.offsetLeft / cw, sy = span.offsetTop / ch;
    const sw = Math.max(0.001, span.offsetWidth / cw);
    const sh = Math.max(0.001, span.offsetHeight / ch);
    if (sx < 0 || sy < 0 || sx > 1 || sy > 1) continue;
    let bg = null;
    try {
      const e = span._smartPdfErase;
      if (e && e.img && e.img.data && e.img.data.length >= 16) {
        const d = e.img.data;
        const buckets = new Map();
        ...
        buckets.forEach((b) => { if (b[0] > bestN) { bestN = b[0]; best = b; } });
        if (best) bg = 'rgb(' + Math.round(best[1] / best[0]) + ',' + Math.round(best[2] / best[0]) + ',' + Math.round(best[3] / best[0]) + ')';
      }
    } catch (err) { bg = null; }
    const cs = span.ownerDocument && span.ownerDocument.defaultView
      ? span.ownerDocument.defaultView.getComputedStyle(span) : null;
    out.push({
      page: n, text: txt,
      dir: span.getAttribute('dir') || (cs ? cs.direction : 'ltr') || 'ltr',
      x: Math.min(0.99, Math.max(0, sx)), y: Math.min(0.99, Math.max(0, sy)),
      w: Math.min(1, sw), h: Math.min(1, sh),
      ow: span._smartPdfOverflowPx ? (span._smartPdfOverflowPx / cw) : 0,
      fontSize: parseFloat(span.style.fontSize || '') || parseFloat(cs ? cs.fontSize : '') || 12,
      fontWeight: span.style.fontWeight || (cs ? cs.fontWeight : '') || '',
      fontStyle: span.style.fontStyle || (cs ? cs.fontStyle : '') || '',
      fontFamily: span.style.fontFamily || (cs ? cs.fontFamily : '') || '',
      color: span.style.color || (cs ? cs.color : '') || '#000000',
      bg: bg
    });
  }
  return out;
}
// One edited run -> small transparent PNG (canvas keeps RTL/Arabic shaping).
function smartPdfV3EditToPng(ed) {
  const scale = 3;
  const fs = Math.max(8, ed.fontSize * scale);
  const c = document.createElement('canvas');
  const meas = document.createElement('canvas').getContext('2d');
  let font = '';
  if (ed.fontStyle && ed.fontStyle !== 'normal') font += ed.fontStyle + ' ';
  if (ed.fontWeight && ed.fontWeight !== 'normal') font += ed.fontWeight + ' ';
  font += fs + 'px ' + (ed.fontFamily || 'sans-serif');
  let tw = 200;
  try {
    meas.font = font;
    tw = Math.max(24, Math.min(2200, Math.ceil(meas.measureText(ed.text).width + fs * 0.6)));
  } catch (e) {}
  const h = Math.round(Math.max(10, fs * 1.5));
  c.width = tw; c.height = h;
  const ctx = c.getContext('2d');
  if (!ctx) throw new Error('text canvas unavailable');
  ctx.font = font;
  ctx.fillStyle = ed.color || '#000000';
  ctx.textBaseline = 'middle';
  try { ctx.direction = (ed.dir === 'rtl') ? 'rtl' : 'ltr'; } catch (e) {}
  ctx.textAlign = (ed.dir === 'rtl') ? 'right' : 'left';
  const txp = (ed.dir === 'rtl') ? (tw - fs * 0.2) : fs * 0.2;
  ctx.fillText(ed.text, txp, h / 2);
  const url = c.toDataURL('image/png');
  try { c.width = 0; c.height = 0; } catch (e) {}
  return { url: url, ratio: tw / h };
}
// Pristine bytes + committed edits burned via the vendored pdf-lib loader.
async function smartPdfV3CurrentPdfBytes() {
  if (!smartPdfV3Bytes) return null;
  const edits = smartPdfV3EditedSpans();
  const pristine = smartPdfV3Bytes.slice ? smartPdfV3Bytes.slice(0) : smartPdfV3Bytes;
  if (!edits.length) return pristine;
  const lib = (typeof smartImportLoadPdfLib === 'function') ? await smartImportLoadPdfLib() : window.PDFLib;
  if (!lib || !lib.PDFDocument) return pristine;
  const pdf = await lib.PDFDocument.load(pristine, { ignoreEncryption: true });
  for (let i = 0; i < edits.length; i++) {
    const ed = edits[i];
    let page = null;
    try { page = pdf.getPage(ed.page - 1); } catch (e) { page = null; }
    if (!page) continue;
    const size = page.getSize();
    const pw = size.width, ph = size.height;
    const bx = Math.max(0, Math.min(pw - 1, ed.x * pw));
    const bw = Math.max(1, Math.min(pw - bx, ed.w * pw));
    const bh = Math.max(1, ed.h * ph);
    const by = Math.max(0, ph - Math.max(0, Math.min(ph, ed.y * ph)) - bh);
    try {
      const png = smartPdfV3EditToPng(ed);
      const img = await pdf.embedPng(png.url);
      // Hide the ORIGINAL glyphs first with the page's own background colour
      // (captured at edit time; white default keeps today's behaviour).
      try {
        const m = /rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i.exec(String(ed.bg || 'rgb(255,255,255)'));
        const cr = Math.min(1, Math.max(0, Number(m ? m[1] : 255) / 255));
        const cg = Math.min(1, Math.max(0, Number(m ? m[2] : 255) / 255));
        const cb = Math.min(1, Math.max(0, Number(m ? m[3] : 255) / 255));
        page.drawRectangle({ x: bx, y: by, width: bw, height: bh, color: lib.rgb(cr, cg, cb) });
      } catch (e) {}
      // NATURAL SIZE (no old-box squeeze): the PNG already carries the true
      // aspect of the replacement text, so scale by HEIGHT (font size) and let
      // the width follow. A longer replacement keeps its shaping/spacing
      // instead of being compressed into the old box (which garbled
      // Arabic/Kurdish/numbers). The draw rect widens only as much as the
      // measured text needs; neighbours were nudged on screen by the same
      // overflow amount (smartPdfV3NudgeNeighbour).
      let ih = bh, iw = ih * Math.max(0.01, png.ratio);
      if (ih > bh * 1.6) { ih = bh * 1.6; iw = ih * Math.max(0.01, png.ratio); }
      if (ed.dir === 'rtl') {
        const owPt = Math.max(0, (ed.ow || 0) * pw);
        const ix = bx + bw - iw;
        const cx = Math.max(0, Math.min(ix, bx + bw - iw + owPt));
        if (owPt > 0.5) {
          try {
            const m2 = /rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i.exec(String(ed.bg || 'rgb(255,255,255)'));
            page.drawRectangle({ x: cx, y: by, width: (bx + bw + owPt) - cx, height: bh,
              color: lib.rgb(Math.min(1, Math.max(0, Number(m2 ? m2[1] : 255) / 255)),
                Math.min(1, Math.max(0, Number(m2 ? m2[2] : 255) / 255)),
                Math.min(1, Math.max(0, Number(m2 ? m2[3] : 255) / 255))) });
          } catch (e2) {}
        }
        page.drawImage(img, { x: cx, y: by + Math.max(0, (bh - ih) / 2), width: iw, height: ih });
      } else {
        const owPt = Math.max(0, (ed.ow || 0) * pw);
        if (owPt > 0.5) {
          try {
            const m2 = /rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i.exec(String(ed.bg || 'rgb(255,255,255)'));
            page.drawRectangle({ x: bx, y: by, width: Math.min(pw - bx, iw), height: bh,
              color: lib.rgb(Math.min(1, Math.max(0, Number(m2 ? m2[1] : 255) / 255)),
                Math.min(1, Math.max(0, Number(m2 ? m2[2] : 255) / 255)),
                Math.min(1, Math.max(0, Number(m2 ? m2[3] : 255) / 255))) });
          } catch (e2) {}
        }
        page.drawImage(img, { x: bx, y: by + Math.max(0, (bh - ih) / 2), width: iw, height: ih });
      }
    } catch (e) {}
  }
  const out = await pdf.save();
  return out && out.buffer ? out.buffer.slice(0) : out;
}
async function smartPdfV3ShareCurrent() {
  if (smartPdfV3ShareBusy) return;
  if (!smartPdfV3Bytes || !smartPdfV3File) { smartPdfV3SetError('smartPdfUploadError'); return; }
  smartPdfV3ShareBusy = true;
  try {
    const bytes = await smartPdfV3CurrentPdfBytes();
    if (!bytes) { smartPdfV3SetError('smartPdfUploadError'); return; }
    const blob = new Blob([bytes], { type: 'application/pdf' });
    const base = String((smartPdfV3File && smartPdfV3File.name) || 'document.pdf').replace(/\.pdf$/i, '');
    const filename = (base || 'document') + '.pdf';
    try {
  ...
  } catch (e) { smartPdfV3SetError('smartPdfUploadError'); }
  finally { smartPdfV3ShareBusy = false; }
}
function smartPdfV3OpenPicker() {
...
function smartPdfV3Wire() {
  ...
  if (shareBtn) shareBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    try { smartPdfV3ShareCurrent(); } catch (err) {}
  });
  const replaceBtn = smartPdfV3El('smartPdfReplaceBtn');
  if (replaceBtn) replaceBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    smartPdfV3OpenPicker();
  });
  // Icon-only buttons: keep aria-label in sync with localized title.
  try {
    const ids = ['smartPdfShareBtn', 'smartPdfReplaceBtn'];
    const sync = () => {
      for (let i = 0; i < ids.length; i++) {
        const b = smartPdfV3El(ids[i]);
        if (b && b.getAttribute('title')) b.setAttribute('aria-label', b.getAttribute('title'));
      }
    };
    sync();
    if (typeof MutationObserver !== 'undefined') {
      ...
      for (let i = 0; i < ids.length; i++) {
        const b = smartPdfV3El(ids[i]);
        if (b) mo.observe(b, { attributes: true, attributeFilter: ['title'] });
      }
    }
  } catch (e) {}
...
function pdfV1OpenFile(file) {
  ...
  // Loaded state: hide header + upload UI, slim toolbar + full-workspace viewer.
  try {
    const wsEl = document.getElementById('pdfReportsWorkspace');
    const modalEl = wsEl && wsEl.querySelector('.pdf-reports-workspace');
    if (modalEl) modalEl.classList.add('pdf-reports-loaded');
  } catch (e) { /* layout-only */ }
...
function pdfV1WireImport() {
  ...
  const closeBtn = document.getElementById('pdfV1CloseBtn');
...
let pdfV1P2SigInk = false;
 // ---- Stamp color + opacity state (Stamp Options popover) -------------------
 // The stamp keeps its original file / dataURL in it.src. color (hex, or null =
 // the original look) and opacity (0..1, default 1) are per-stamp properties.
 const PDFV1_STAMP_LONG_PRESS_MS = 650; // short mobile alternative (never 3s)
 const PDFV1_STAMP_DOUBLE_TAP_MS = 400; // single click/tap stays "select only"
 let pdfV1StampOptItem = null;          // stamp item whose options are open
 let pdfV1StampOptAnchor = null;        // its DOM element (popover positioning)
 let pdfV1StampLastTap = { id: null, t: 0, type: '' };

 // Tint a raster stamp (PNG/JPG) to colorHex WITHOUT touching the original
 // source: the result is a derived PNG dataURL whose alpha matches the stamp
 // (globalCompositeOperation 'source-in'). Image decoding is async, hence the
 // callback.
 function pdfV1P2TintStamp(src, colorHex, cb) {
   const done = (typeof cb === 'function') ? cb : function () {};
   if (!src || !colorHex) { done(src); return; }
   try {
     const im = new Image();
     im.onload = () => {
       try {
         ...
         ctx.globalCompositeOperation = 'source-in'; // keep the stamp's own alpha
...
function pdfV1P2AfterOpen(file) {
  ...
  pdfV1P2Sel = null;
  pdfV1P2PageCount = 1;
  pdfV1P2StampTintCache = {};   // new document: drop derived stamp tints
  pdfV1P2StampTintPending = {};
  try { pdfV1StampOptClose(); } catch (e) {}
...
function pdfV1P2PaintPager() {
  // Page navigation control (label + prev/next arrows) removed from the toolbar.
  // This function now only syncs the Delete button state.
  const del = pdfV1P2El('pdfV1DelBtn');
  if (del) del.disabled = !pdfV1P2Sel;
}
function pdfV1P2SigOpacity(it) {
  const n = Number(it && it.sigOpacity);
  if (!isFinite(n)) return 1;
  return Math.min(1, Math.max(0, n));
}
function pdfV1P2Clamp(it) {
  // Signature images stay FULLY inside the page: no part may leave the layer.
  // Other items keep the legacy tolerant clamp (never touch Stamp/Text/etc).
  if (pdfV1P2IsSig(it)) {
    let w = Math.min(0.90, Math.max(0.08, Number(it.w) || 0.2));
    it.w = w;
    const layer = pdfV1P2El('pdfV1Layer');
    const lw = Math.max(1, (layer && layer.clientWidth) || 1);
    const lh = Math.max(1, (layer && layer.clientHeight) || 1);
    const asp = (it.aspect && it.aspect > 0) ? it.aspect : 1;
    let h = w * (lw / lh) / asp;
    h = Math.min(0.90, Math.max(0.05, h || 0.15));
    // Re-derive w from clamped h so aspect never distorts.
    w = Math.min(0.90, Math.max(0.08, h * asp * (lh / lw)));
    it.w = w; it.h = h;
    it.x = Math.min(Math.max(0, 1 - w), Math.max(0, Number(it.x) || 0));
    it.y = Math.min(Math.max(0, 1 - h), Math.max(0, Number(it.y) || 0));
    return it;
  }
  const keep = 0.05;
...
function pdfV1P2IsSig(it) {
  return !!(it && it.kind === 'img' && it.sig === true && it.stamp !== true);
}
function pdfV1P2Render() {
  ...
  pdfV1P2List().forEach((it) => {
    ...
    const sig = pdfV1P2IsSig(it);
    el.className = 'pdfv1-item' + (pdfV1P2Sel === it.id ? ' pdfv1-selected' : '') + (sig ? ' is-signature' : '');
    ...
    if (it.kind === 'text') {
      ...
      img.alt = '';
      img.draggable = false;
      // Stamp: show the derived tint (the original image is untouched) and apply
      // the per-stamp opacity to this image ONLY. Selection frame and the 4
      // resize handles stay fully visible.
      img.src = it.color ? pdfV1P2StampTintedSrc(it) : it.src;
      if (it.stamp) img.style.opacity = String(pdfV1P2StampOpacity(it));
      // Signature-only opacity (per-signature, never touches stamp/PDF/text).
      // Only set inline opacity when translucent so the default stays ''.
      if (sig && pdfV1P2SigOpacity(it) !== 1) img.style.opacity = String(pdfV1P2SigOpacity(it));
      el.appendChild(img);
    }
    // Signatures always paint above stamps/images so "signature over stamp" works.
    if (sig) el.style.zIndex = '20';
    if (pdfV1P2IsSig(it)) {
      // Signature selection UI ONLY: delete X (top corner) + 4 corner resize
      // handles. No background/border/shadow is added to the PNG itself;
      // the thin selection rectangle comes from .pdfv1-selected. Export ignores
      // these controls because it reads pdfV1P2Items (geometry + src) only.
      const del = document.createElement('button');
      del.type = 'button';
      del.className = 'pdfv1-sig-del';
      del.textContent = '×';
    ...
    const hdAll = el.querySelectorAll('.pdfv1-handle');
    pdfV1P2Gest(el, it, hdAll);
    layer.appendChild(el);
  });
  pdfV1P2PaintPager();
  try { if (pdfV1P2Sel) pdfV1SigOptPosition(); } catch (e) {}
}
function pdfV1P2Select(id) {
  ...
  pdfV1P2PaintPager();
  try { pdfV1SigOptSync(); } catch (e) {}
}
// ---- Signature opacity popover (signature ONLY) ---------------------------
// Small popover anchored near the selected signature with a single 0..100
// slider. It never touches stamps/text/PDF: only it.sigOpacity of the selected
// signature item. Auto-opens on signature select, closes on deselect/delete.
function pdfV1SigOptText() {
  const ar = pdfV1P2Lang().indexOf('ar') === 0;
  if (ar) return { title: 'شفافية التوقيع', opacity: 'الشفافية', close: 'إغلاق' };
  return { title: 'Signature opacity', opacity: 'Opacity', close: 'Close' };
}
function pdfV1SigOptSelItem() {
  if (!pdfV1P2Sel) return null;
  const list = pdfV1P2List();
  for (let i = 0; i < list.length; i++) {
    if (String(list[i].id) === String(pdfV1P2Sel) && pdfV1P2IsSig(list[i])) return list[i];
  }
  return null;
}
function pdfV1SigOptPaintValue() {
  const it = pdfV1SigOptSelItem();
  const pct = Math.round(pdfV1P2SigOpacity(it || { sigOpacity: 1 }) * 100);
  const val = pdfV1P2El('pdfV1SigOpacityVal');
  if (val) val.textContent = pct + '%';
  const slider = pdfV1P2El('pdfV1SigOpacity');
  if (slider) slider.value = String(it ? pct : 100);
  const t = pdfV1SigOptText();
  const title = pdfV1P2El('pdfV1SigOptTitle');
  if (title) title.textContent = t.title;
  const lab = pdfV1P2El('pdfV1SigOptOpacityText');
  if (lab) lab.textContent = t.opacity;
...
function pdfV1SigOptPosition() {
  ...
  const ph = Math.max(1, pop.offsetHeight || 90);
  let r = null;
  try {
    const anchor = pdfV1P2Sel ? pdfV1P2ItemEl(pdfV1P2Sel) : null;
    r = anchor ? anchor.getBoundingClientRect() : null;
  } catch (e) { r = null; }
...
function pdfV1SigOptSync() {
  const pop = pdfV1P2El('pdfV1SigOptions');
  if (!pop) return;
  const it = pdfV1SigOptSelItem();
  if (!it) { pop.hidden = true; return; }
  pdfV1SigOptPaintValue();
  pop.hidden = false;
  pop.style.left = '0px';
  pop.style.top = '0px';
  pdfV1SigOptPosition();
}
function pdfV1SigOptSetOpacity(pct) {
  const it = pdfV1SigOptSelItem();
  if (!it) return;
  const n = Number(pct);
  it.sigOpacity = isFinite(n) ? Math.min(1, Math.max(0, n / 100)) : 1;
  const img = pdfV1P2ItemImg(it.id);
  if (img) {
    if (pdfV1P2SigOpacity(it) === 1) img.style.opacity = '';
    else img.style.opacity = String(pdfV1P2SigOpacity(it));
  }
  pdfV1SigOptPaintValue();
}
function pdfV1SigOptWire() {
  const pop = pdfV1P2El('pdfV1SigOptions');
  if (!pop || pop.dataset.pdfv1wired === '1') return;
  const slider = pdfV1P2El('pdfV1SigOpacity');
  const close = pdfV1P2El('pdfV1SigOptClose');
  if (!slider || !close) return;
  pop.dataset.pdfv1wired = '1';
  slider.addEventListener('input', () => pdfV1SigOptSetOpacity(slider.value));
  ...
  // Signature-only "click outside deselects". The layer is pointer-events:none,
  // so real taps/clicks on empty page area land on the PDF iframe (or tools) and
  // never reach the layer's own pointerdown handler. This capture listener only
  // deselects the SELECTED SIGNATURE; selecting other items (stamp/text/date)
  // is untouched and still runs afterwards in its own handler.
  document.addEventListener('pointerdown', (e) => {
    let sel = null;
    try { sel = document.querySelector('#pdfV1Layer .pdfv1-item.is-signature.pdfv1-selected'); } catch (err) { sel = null; }
    if (!sel) return;
    if (e.target && sel.contains(e.target)) return;
    if (pop && !pop.hidden && e.target && pop.contains(e.target)) return;
    try { pdfV1P2Select(null); } catch (err) {}
  }, true);
  window.addEventListener('resize', () => { try { pdfV1SigOptPosition(); } catch (e) {} });
  window.addEventListener('scroll', () => { try { pdfV1SigOptPosition(); } catch (e) {} }, true);
}
function pdfV1P2Gest(el, it, hd) {
  el.addEventListener('pointerdown', (e) => {
    if ((e.target && e.target.closest && (e.target.closest('.pdfv1-handle') || e.target.closest('.pdfv1-sig-del'))) || Array.from(hd).some((h) => h === e.target)) return;
    e.preventDefault();
    pdfV1P2Select(it.id); // existing behavior: single click/tap selects + shows handles
    // Stamp items ONLY: double-click (mouse) / double-tap (touch) opens Stamp
    // Options, and a short ~650ms long-press is the mobile alternative. A single
    // click/tap still only selects and never opens the popover.
    const stampIt = (it.kind === 'img' && it.stamp === true);
    ...
    if (stampIt) {
      const now = Date.now();
      const dbl = (pdfV1StampLastTap.id === it.id && pdfV1StampLastTap.type === ptype &&
        (now - pdfV1StampLastTap.t) <= PDFV1_STAMP_DOUBLE_TAP_MS);
      pdfV1StampLastTap = { id: it.id, t: now, type: ptype };
      if (dbl) { pdfV1StampOptOpen(it, el); return; } // 2nd tap: open, never drag
      if (ptype !== 'mouse') {
        lpTimer = setTimeout(() => {
          lpTimer = null;
          lpFired = true;
          pdfV1StampOptOpen(it, el);
        }, PDFV1_STAMP_LONG_PRESS_MS);
      }
    }
    const r = pdfV1P2Rect();
    ...
    const mv = (ev) => {
      if (lpFired) return; // the press became a long-press: do not move the stamp
      const far = (Math.abs(ev.clientX - sx) > 6 || Math.abs(ev.clientY - sy) > 6);
      if (far && !dragged) {
        dragged = true;
        pdfV1StampLastTap = { id: null, t: 0, type: '' }; // a real drag is not a tap
      }
...
function pdfV1P2AddImg(src, aspect, isStamp) {
  const layer = pdfV1P2El('pdfV1Layer');
  const lw = (layer && layer.clientWidth) || 600;
  const lh = (layer && layer.clientHeight) || 700;
  const asp = aspect && aspect > 0 ? aspect : 1;
  const w = isStamp === 'sig' ? 0.42 : 0.34;
  let h = w * (lw / Math.max(1, lh)) / asp;
  if (!(h > 0) || h > 0.6) h = isStamp === 'sig' ? 0.24 : 0.22;
  const it = { id: pdfV1P2Seq++, kind: 'img', x: (1 - w) / 2, y: 0.36, w: w, h: h, src: src, text: '', aspect: asp };
  // Stamp-only properties (Stamp color + opacity). The ORIGINAL image (it.src) is
  // never modified: color/opacity are properties of the stamp item itself.
  if (isStamp === true) { it.stamp = true; it.color = null; it.opacity = 1; }
  // Signature-only flag (drawn signature + uploaded signature image) + its own
  // opacity property (0..1, default 1). Plain image/logo uploads and stamps
  // never get it, so their UI is unchanged.
  if (isStamp === 'sig') { it.sig = true; it.sigOpacity = 1; }
  pdfV1P2Clamp(it);
  pdfV1P2List().push(it);
  pdfV1P2Select(it.id);
  pdfV1P2Render();
}
// ---- Stamp derived tint (the original image file is never replaced) --------
// One tinted PNG dataURL per stamp id, rebuilt only when its color changes.
let pdfV1P2StampTintCache = {};
let pdfV1P2StampTintPending = {};
function pdfV1P2ItemEl(id) {
  try { return document.querySelector('#pdfV1Layer .pdfv1-item[data-pid="' + String(id) + '"]'); } catch (e) { return null; }
}
function pdfV1P2ItemImg(id) {
  const el = pdfV1P2ItemEl(id);
  return el ? el.querySelector('img') : null;
}
// Opacity is a per-stamp property, clamped to 0..1; the default is 1 (100%).
function pdfV1P2StampOpacity(it) {
  const n = Number(it && it.opacity);
  if (!isFinite(n)) return 1;
  return Math.min(1, Math.max(0, n));
}
// Returns the original src while the tinted derivative is still decoding, then
// swaps only that stamp's <img> in place: no re-render, so position, size,
// aspect ratio and selection stay exactly where they were.
function pdfV1P2StampTintedSrc(it) {
  if (!it || it.kind !== 'img') return '';
  if (!it.color) return it.src;
  const key = String(it.id);
  const color = it.color;
  const hit = pdfV1P2StampTintCache[key];
  if (hit && hit.color === color && hit.src) return hit.src;
  if (pdfV1P2StampTintPending[key] !== color) {
    pdfV1P2StampTintPending[key] = color;
    pdfV1P2TintStamp(it.src, color, (tinted) => {
      delete pdfV1P2StampTintPending[key];
      if (!tinted) return;
      pdfV1P2StampTintCache[key] = { color: color, src: tinted };
      if (it.color !== color) return; // the color changed again meanwhile
      const live = pdfV1P2ItemImg(it.id);
      if (live) live.src = tinted;
    });
  }
  return it.src;
}
// ---- Stamp Options popover — Color + Opacity ONLY --------------------------
// A small popover anchored to the selected stamp. It never deletes, moves or
// resizes the stamp, and it never touches the original image file.
function pdfV1StampOptText() {
  const ar = pdfV1P2Lang().indexOf('ar') === 0;
  if (ar) {
    return { title: 'خيارات الختم', color: 'لون الختم', opacity: 'الشفافية', reset: 'إعادة تعيين',
      close: 'إغلاق', '#d93838': 'أحمر', '#2563eb': 'أزرق', '#16a34a': 'أخضر', '#f59e0b': 'برتقالي',
      '#9333ea': 'بنفسجي', '#78716c': 'رمادي', '#000000': 'أسود', '#ffffff': 'أبيض' };
  }
  return { title: 'Stamp options', color: 'Stamp color', opacity: 'Opacity', reset: 'Reset',
    close: 'Close', '#d93838': 'Red', '#2563eb': 'Blue', '#16a34a': 'Green', '#f59e0b': 'Orange',
    '#9333ea': 'Purple', '#78716c': 'Gray', '#000000': 'Black', '#ffffff': 'White' };
}
function pdfV1StampOptApplyText() {
  const L = pdfV1StampOptText();
  const set = (id, v) => { const n = pdfV1P2El(id); if (n) n.textContent = v; };
  set('pdfV1StampOptTitle', L.title);
  set('pdfV1StampOptColorLabel', L.color);
  set('pdfV1StampOptOpacityText', L.opacity);
  set('pdfV1StampReset', L.reset);
  const pop = pdfV1P2El('pdfV1StampOptions');
  const close = pdfV1P2El('pdfV1StampOptClose');
  if (close) close.setAttribute('aria-label', L.close);
  if (pop) {
    pop.setAttribute('aria-label', L.title);
    Array.prototype.forEach.call(pop.querySelectorAll('.pdfv1-stamp-swatch'), (b) => {
      const k = String(b.getAttribute('data-color') || '').toLowerCase();
      if (L[k]) b.setAttribute('title', L[k]);
    });
  }
}
function pdfV1StampOptIsOpen() {
  const pop = pdfV1P2El('pdfV1StampOptions');
  return !!(pop && pop.hidden === false && pdfV1StampOptItem);
}
// Reflect the selected stamp's color/opacity into the popover controls.
function pdfV1StampOptPaintValue() {
  const it = pdfV1StampOptItem;
  const pct = Math.round(pdfV1P2StampOpacity(it) * 100);
  const val = pdfV1P2El('pdfV1StampOpacityVal');
  if (val) val.textContent = pct + '%';
  const slider = pdfV1P2El('pdfV1StampOpacity');
  if (slider) slider.value = String(pct);
  const picker = pdfV1P2El('pdfV1StampColorPicker');
  if (picker && it && it.color) picker.value = it.color;
  const pop = pdfV1P2El('pdfV1StampOptions');
  if (pop) {
    Array.prototype.forEach.call(pop.querySelectorAll('.pdfv1-stamp-swatch'), (b) => {
      const k = String(b.getAttribute('data-color') || '').toLowerCase();
      b.classList.toggle('is-active', !!(it && it.color) && k === String(it.color).toLowerCase());
    });
  }
}
// Place the popover next to the stamp: it flips to the other side and clamps so
// it can never leave the viewport (no horizontal overflow on small screens).
function pdfV1StampOptPosition() {
  const pop = pdfV1P2El('pdfV1StampOptions');
  if (!pop || pop.hidden) return;
  const gap = 8;
  const minTop = Math.max(gap, eqAdBandBottom() + gap); // below the reserved ad band
  ...
  const ph = Math.max(1, pop.offsetHeight || 180);
  let r = null;
  try { r = pdfV1StampOptAnchor ? pdfV1StampOptAnchor.getBoundingClientRect() : null; } catch (e) { r = null; }
  let left = r ? (r.left + r.width + gap) : (vw - pw - gap);
...
// Instant feedback: only this stamp's <img> changes (position, size, aspect
// ratio, selection and the 4 resize handles all stay untouched).
function pdfV1StampOptLive() {
  const it = pdfV1StampOptItem;
  if (!it) return;
  const img = pdfV1P2ItemImg(it.id);
  if (img) {
    img.src = it.color ? pdfV1P2StampTintedSrc(it) : it.src;
    img.style.opacity = String(pdfV1P2StampOpacity(it));
  }
  pdfV1StampOptPaintValue();
  pdfV1StampOptPosition();
}
function pdfV1StampOptOpen(it, el) {
  const pop = pdfV1P2El('pdfV1StampOptions');
  if (!pop || !it || it.kind !== 'img' || it.stamp !== true) return false;
  pdfV1StampOptItem = it;
  pdfV1StampOptAnchor = el || pdfV1P2ItemEl(it.id) || null;
  pdfV1StampOptApplyText();
  pdfV1StampOptPaintValue();
  pop.hidden = false;
  pop.style.left = '0px';
  pop.style.top = '0px';
  // While the options are open the overlay layer accepts pointer events so an
  // outside click/tap lands in this document (instead of the PDF iframe) and can
  // close the popover. Stamp items keep their own pointer events (drag/resize).
  const layer = pdfV1P2El('pdfV1Layer');
  if (layer) layer.style.pointerEvents = 'auto';
  pdfV1StampOptPosition(); // measure, then place (no first-paint jump)
  return true;
}
// Close only the popover: the stamp keeps its selection, place and size.
function pdfV1StampOptClose() {
  const pop = pdfV1P2El('pdfV1StampOptions');
  if (pop) pop.hidden = true;
  const layer = pdfV1P2El('pdfV1Layer');
  if (layer) layer.style.pointerEvents = '';
  pdfV1StampOptItem = null;
  pdfV1StampOptAnchor = null;
}
function pdfV1StampOptSetColor(hex) {
  const it = pdfV1StampOptItem;
  if (!it) return;
  it.color = hex ? String(hex) : null; // null -> the original image look
  pdfV1StampOptLive();
}
function pdfV1StampOptSetOpacity(pct) {
  const it = pdfV1StampOptItem;
  if (!it) return;
  const n = Number(pct);
  it.opacity = isFinite(n) ? Math.min(1, Math.max(0, n / 100)) : 1;
  pdfV1StampOptLive();
}
// Reset -> back to the original state: original color look + opacity 100%.
function pdfV1StampOptReset() {
  const it = pdfV1StampOptItem;
  if (!it) return;
  it.color = null;
  it.opacity = 1;
  const picker = pdfV1P2El('pdfV1StampColorPicker');
  if (picker) picker.value = '#d93838';
  const slider = pdfV1P2El('pdfV1StampOpacity');
  if (slider) slider.value = '100';
  pdfV1StampOptLive();
}
// Single wiring point (called once from pdfV1P2Wire). Every lookup is guarded so
// a missing node can never break the existing overlay wiring.
function pdfV1StampOptWire() {
  const pop = pdfV1P2El('pdfV1StampOptions');
  if (!pop || pop.dataset.pdfv1wired === '1') return;
  const picker = pdfV1P2El('pdfV1StampColorPicker');
  const slider = pdfV1P2El('pdfV1StampOpacity');
  const reset = pdfV1P2El('pdfV1StampReset');
  const close = pdfV1P2El('pdfV1StampOptClose');
  if (!picker || !slider || !reset || !close) return;
  pop.dataset.pdfv1wired = '1';
  picker.addEventListener('input', () => pdfV1StampOptSetColor(picker.value));
  slider.addEventListener('input', () => pdfV1StampOptSetOpacity(slider.value));
  reset.addEventListener('click', pdfV1StampOptReset);
  close.addEventListener('click', pdfV1StampOptClose);
  Array.prototype.forEach.call(pop.querySelectorAll('.pdfv1-stamp-swatch'), (b) => {
    b.addEventListener('click', () => {
      const c = String(b.getAttribute('data-color') || '');
      if (!c) return;
      if (picker) picker.value = c;
      pdfV1StampOptSetColor(c);
    });
  });
  // A press outside closes the popover only: it never deletes or moves the stamp.
  document.addEventListener('pointerdown', (e) => {
    if (!pdfV1StampOptIsOpen()) return;
    const t = e.target;
    if (t && (pop === t || pop.contains(t))) return;
    if (pdfV1StampOptAnchor && t && (pdfV1StampOptAnchor === t || pdfV1StampOptAnchor.contains(t))) return;
    pdfV1StampOptClose();
  }, true);
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && pdfV1StampOptIsOpen()) pdfV1StampOptClose();
  });
  window.addEventListener('resize', () => { if (pdfV1StampOptIsOpen()) pdfV1StampOptPosition(); });
  window.addEventListener('scroll', () => { if (pdfV1StampOptIsOpen()) pdfV1StampOptPosition(); }, true);
}
...
function pdfV1P2Del() {
  ...
  try { delete pdfV1P2StampTintCache[String(pdfV1P2Sel)]; } catch (e) {}
  try { if (pdfV1StampOptItem && String(pdfV1StampOptItem.id) === String(pdfV1P2Sel)) pdfV1StampOptClose(); } catch (e) {}
  pdfV1P2Select(null);
  pdfV1P2Render();
}
...
function pdfV1P2Wire() {
  try { pdfV1StampOptWire(); } catch (e) {} // Stamp Options popover (color + opacity)
  try { pdfV1SigOptWire(); } catch (e) {} // Signature opacity popover (sig only)
  ...
  pdfV1P2El('pdfV1StampInput').addEventListener('change', (e) => {
    ...
    e.target.value = '';
    // Stamp uploads only: flag the item so Stamp Options apply to it. Signature
    // images and drawn signatures keep the plain overlay item (unchanged).
    if (f) pdfV1P2ReadImg(f, (url, a) => pdfV1P2AddImg(url, a, true));
  });
  pdfV1P2El('pdfV1SigFileInput').addEventListener('change', (e) => {
    const f = e.target && e.target.files && e.target.files[0];
    e.target.value = '';
    if (f) pdfV1P2ReadImg(f, (url, a) => pdfV1P2AddImg(url, a, 'sig'));
  });
  ...
  // Page navigation control removed: pdfV1PrevPg / pdfV1NextPg no longer exist.
...
// ---------------------------------------------------------------------------
// Note → PDF letterhead strings (Notes → PDF ONLY).
// Same scoped pattern as PDF_REPORT_I18N above: ONE small per-locale map that
// reuses the app's existing locale selection (state.locale) — no second
// translation system. Product names (EQ7 / EQ7.APP) are deliberately not
// translated. Weekday names are listed per locale instead of being read from
// Intl so a Kurdish (or any other) PDF never falls back to an English day name
// on a browser whose ICU data lacks that locale.
// ---------------------------------------------------------------------------
const NOTE_PDF_BRAND = 'EQ7';
const NOTE_PDF_DEFAULT_TITLE = 'EQ7.APP';
const NOTE_PDF_I18N = {
  en: { am: 'AM', pm: 'PM',
        weekdays: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
        footer: 'EQ7 - Smart Calculator • Calculations • Currency Conversion • Financial Tools • History & Reports • Professional PDF' },
  ar: { am: 'ص', pm: 'م',
        weekdays: ['الأحد', 'الاثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت'],
        footer: 'EQ7 • حاسبة ذكية للحسابات والتحويل والعملات • الأدوات المالية • السجل والتقارير • PDF احترافي' },
  ku: { am: 'پ.ن', pm: 'د.ن',
        weekdays: ['یەکشەممە', 'دووشەممە', 'سێشەممە', 'چوارشەممە', 'پێنجشەممە', 'هەینی', 'شەممە'],
        footer: 'EQ7 - حیسابکەری زیرەک • حیسابکردن • گۆڕینی دراو • ئامرازە داراییەکان • مێژوو و ڕاپۆرتەکان • PDFی پیشەیی' },
  fr: { am: 'AM', pm: 'PM',
        weekdays: ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi'],
        footer: 'EQ7 - Calculatrice intelligente • Calculs • Conversion de devises • Outils financiers • Historique et rapports • PDF professionnel' },
  de: { am: 'AM', pm: 'PM',
        weekdays: ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'],
        footer: 'EQ7 - Intelligenter Rechner • Berechnungen • Währungsumrechnung • Finanzwerkzeuge • Verlauf & Berichte • Professionelles PDF' },
  es: { am: 'AM', pm: 'PM',
        weekdays: ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'],
        footer: 'EQ7 - Calculadora inteligente • Cálculos • Conversión de divisas • Herramientas financieras • Historial e informes • PDF profesional' },
  ru: { am: 'AM', pm: 'PM',
        weekdays: ['воскресенье', 'понедельник', 'вторник', 'среда', 'четверг', 'пятница', 'суббота'],
        footer: 'EQ7 - Умный калькулятор • Вычисления • Конвертация валют • Финансовые инструменты • История и отчёты • Профессиональный PDF' },
  tr: { am: 'AM', pm: 'PM',
        weekdays: ['Pazar', 'Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma', 'Cumartesi'],
        footer: 'EQ7 - Akıllı Hesap Makinesi • Hesaplamalar • Döviz Dönüşümü • Finansal Araçlar • Geçmiş ve Raporlar • Profesyonel PDF' }
};

function notePdfText(locale) {
  return NOTE_PDF_I18N[locale] || NOTE_PDF_I18N.en;
}

// NOTE-PDF footer RTL ONLY (ar/ku): escape the footer line, then wrap the
// Latin runs (EQ7 / PDF) in <span class="f-ltr"> so the BiDi algorithm keeps
// them LTR-isolated inside the RTL sentence (correct order, no flip/overlap).
// LTR locales return the plain escaped line (byte-identical to before).
function notePdfFooterHtml(footerLine, locale) {
  const safe = escapeHtml(String(footerLine || ''));
  if (locale !== 'ar' && locale !== 'ku') return safe;
  return safe.replace(/(EQ7|PDF)/g, '<span class="f-ltr">$1</span>');
}

// Letterhead stamp of ONE moment: 12-hour time, dd/MM/yyyy date and the
// localized weekday name — the SAME moment (the PDF generation/send time) is
// used for all three. Built by hand (no Intl) so the digits always stay
// Western 0-9 in every language, including Arabic and Kurdish, and the parts
// are always in the required order: time, then date, then the day alone (the
// date is never repeated beside the day).
function notePdfStampParts(date, locale) {
  const txt = notePdfText(locale);
  const d = (date instanceof Date && !isNaN(date.getTime())) ? date : new Date();
  const h24 = d.getHours();
  const h12 = (h24 % 12) || 12;
  const pad = (n) => (n < 10 ? '0' + n : String(n));
  return {
    time: pad(h12) + ':' + pad(d.getMinutes()) + ' ' + (h24 < 12 ? (txt.am || 'AM') : (txt.pm || 'PM')),
    date: pad(d.getDate()) + '/' + pad(d.getMonth() + 1) + '/' + d.getFullYear(),
    day: (txt.weekdays && txt.weekdays[d.getDay()]) || ''
  };
}
...
function buildNotePdfHtml(note) {
  const t = translations[state.locale] || translations.en;
  const pdfText = notePdfText(state.locale);
  // Whole-document direction follows the SELECTED APPLICATION LANGUAGE (the
  // same ar/ku -> rtl rule the History report already uses).
  const appDir = pdfReportLocaleDir(state.locale);
  // A title counts as "entered" only when the user actually typed one: the
  // editor's untitled placeholder is not a real title, so an untitled note
  // shows the product default (EQ7.APP) in the letterhead centre instead.
  const rawTitle = (note && typeof note.title === 'string') ? note.title.trim() : '';
  const untitledLabel = String(t.untitled || 'Untitled');
  const userTitle = (rawTitle && rawTitle !== untitledLabel) ? rawTitle : '';
  const headerTitle = userTitle || NOTE_PDF_DEFAULT_TITLE;
  const title = userTitle || untitledLabel;
  ...
  // Letterhead stamp = the ACTUAL moment this PDF is generated/sent (not the
  // note's last edit time), rendered in the app language.
  const stamp = notePdfStampParts(new Date(), state.locale);
  const footerLine = String(pdfText.footer || NOTE_PDF_I18N.en.footer);
  const titleDir = pdfCellDir(headerTitle);
  ...
  // the note's own title (falling back to the localised untitled label so an
  // export never blocks), the note's own Style/Frame, and the Company Profile
  // is never injected into a note export.
  ...
  const wm = (docOptions.watermark && docOptions.watermark.on && String(docOptions.watermark.text || '').trim())
    ...
    : null;
  // Presentation options: the Header switch shows/hides the letterhead
  // (EQ7 | title | time·date·day); when it is off the title is kept visible as
  // a heading above the note body, so a note never loses its title.
  const showHeader = docOptions.header !== false;
  const showFooter = docOptions.footer !== false;

  const html = `<!DOCTYPE html>
<html lang="${t.locale || state.locale || 'en'}" dir="${appDir}">
<head>
<meta charset="utf-8">
<title>EQ7 Note PDF</title>
<style>
  .eq-note-report { color:#000000; background:#ffffff; font-family:"Segoe UI", Tahoma, "Noto Sans", "Noto Naskh Arabic", Arial, sans-serif; font-size:12.5px; line-height:1.65; max-width:100%; overflow-wrap:anywhere; }
  .eq-note-header { border:none; background:none; padding:2px 10px 10px; margin:0 0 12px; page-break-after:avoid; break-after:avoid; }
  .eq-note-header h1 { margin:0; font-size:16px; color:#000000; letter-spacing:0.2px; }
  /* Letterhead: physical LEFT (EQ7) | CENTRE (note title) | RIGHT (time, date,
     weekday). direction:ltr keeps the brand on the left and the stamp on the
     right in EVERY language (RTL included); only the text itself is localized.
     The grid keeps the centre column truly centred on the page. */
  .eq-note-header-grid { display:grid; grid-template-columns:1fr auto 1fr; align-items:start; column-gap:12px; direction:ltr; }
  .eq-note-brand { justify-self:start; text-align:left; font-size:15px; font-weight:700; color:#000000; letter-spacing:0.3px; white-space:nowrap; }
  .eq-note-header .eq-note-title { margin:0; justify-self:center; text-align:center; font-size:16.5px; font-weight:700; max-width:100%; }
  ...
  .eq-note-body h1, .eq-note-body h2, .eq-note-body h3 { color:#000000; margin:12px 0 6px; line-height:1.4; text-align:center; word-break:break-word; overflow-wrap:anywhere; page-break-after:avoid; break-after:avoid; }
  .eq-note-body h1:first-child, .eq-note-body h2:first-child, .eq-note-body h3:first-child { margin-top:0; }
  .eq-note-body h1 { font-size:16.5px; font-weight:700; }
  .eq-note-body h2 { font-size:14.5px; font-weight:700; }
  .eq-note-body h3 { font-size:13px; font-weight:700; }
  .eq-note-body ul, .eq-note-body ol { margin:6px 0 10px; padding-inline-start:24px; padding-inline-end:0; text-align:start; max-width:100%; }
  .eq-note-body li { margin:2px 0; line-height:1.65; word-break:break-word; overflow-wrap:anywhere; text-align:start; unicode-bidi:plaintext; }
  .eq-pdf-checklist { text-align:start; max-width:100%; margin:6px 0 10px; }
  .eq-pdf-divider { border:none; border-top:1px solid #cbd5e1; margin:10px auto; width:60%; }
  ...
  .eq-note-footer { margin:22px auto 6px; padding-top:0; border:none; background:none; text-align:center; page-break-inside:avoid; break-inside:avoid; line-height:1.6; max-width:100%; }
  .eq-note-footer::before { content:""; display:block; width:52%; max-width:340px; margin:0 auto 8px; border-top:1px solid #94a3b8; }
  /* ONE centred footer line (never three stacked lines): wrap allowed so long
     translations never overflow the A4 text column; small type keeps it compact. */
  .eq-note-footer .fb { display:block; font-size:9px; line-height:1.6; color:#475569; font-weight:500; letter-spacing:0.1px; white-space:normal; word-break:break-word; overflow-wrap:anywhere; text-align:center; max-width:100%; margin:0 auto; padding:0 8px; }
  /* NOTE-PDF footer RTL fix (ar/ku only): letter-spacing breaks Arabic/Kurdish
     joining + html2canvas BiDi layout (overlapping/garbled glyphs). LTR keeps
     the exact design above; RTL uses zero spacing with proper bidi isolation. */
  .eq-note-footer[dir="rtl"] .fb { letter-spacing:0; word-spacing:0; unicode-bidi:plaintext; direction:rtl; }
  .eq-note-footer[dir="rtl"] .fb .f-ltr { display:inline-block; direction:ltr; unicode-bidi:isolate; letter-spacing:0; }
  /* Retained helpers (note-PDF stylesheet contract): no longer emitted — the
     footer is a single line — but kept so the exported page styles stay stable. */
...
<body>
  <div class="eq-note-report${tplClass}${pdfStyleClass}${pdfFrameClass}" id="note-report" dir="${appDir}">
    ${wm ? '<div class="eq-pdf-watermark" style="opacity:' + wm.opacity + '">' + escapeHtml(wm.text) + '</div>' : ''}
    ${showHeader ? `<div class="eq-note-header">
      <div class="eq-note-header-grid">
        <div class="eq-note-brand" dir="ltr">${escapeHtml(NOTE_PDF_BRAND)}</div>
        <h1 class="eq-note-title" dir="${titleDir}">${escapeHtml(headerTitle)}</h1>
        <div class="eq-note-stamp" dir="ltr">
          <span class="eq-note-stamp-line">${escapeHtml(stamp.time)}</span>
          <span class="eq-note-stamp-line">${escapeHtml(stamp.date)}</span>
          <span class="eq-note-stamp-line">${escapeHtml(stamp.day)}</span>
        </div>
      </div>
    </div>` : `<h2 class="eq-note-title" dir="${pdfCellDir(title)}">${escapeHtml(title)}</h2>`}
    <div class="eq-note-body"${bodyDir === 'rtl' ? ' dir="rtl"' : ''}>${bodyHtml}</div>
    ${showFooter ? `<div class="eq-note-footer" dir="${appDir}">
      <div class="fb">${notePdfFooterHtml(footerLine, state.locale)}</div>
    </div>` : ''}
  </div>
</body>
</html>`;
...
const EQ_PRINT_ISOLATION_CSS = [
  ...
  '  /* Global top ad banner: guaranteed 100% hidden on every print path. */',
  '  .global-top-ad-banner, .ad-placeholder, #adPlaceholder { display:none !important; visibility:hidden !important; opacity:0 !important; height:0 !important; max-height:0 !important; overflow:hidden !important; background:transparent !important; border:0 !important; margin:0 !important; padding:0 !important; }',
...
function openInstallModal() {
  // PWA install flow: use the native install prompt on Windows/Android when
  // available, otherwise show the in-app iOS/iPadOS instructions modal.
  if (typeof window !== 'undefined' && window.__pwaDeferredPrompt) {
    try {
      const deferred = window.__pwaDeferredPrompt;
      const doPrompt = () => {
        try {
          const result = deferred.prompt();
          if (result && typeof result.catch === 'function') result.catch(() => {});
        } catch (e) { /* native prompt unavailable */ }
      };
      if (deferred.userChoice && typeof deferred.userChoice.then === 'function') {
        doPrompt();
        try {
          deferred.userChoice.then(() => { window.__pwaDeferredPrompt = null; }).catch(() => { window.__pwaDeferredPrompt = null; });
        } catch (e) { window.__pwaDeferredPrompt = null; }
      } else {
        doPrompt();
...
function initialize() {
  ...
  // PWA install flow (Windows/Android): capture beforeinstallprompt so the
  // header Install button can open the native install prompt. Best effort —
  // iOS/iPadOS simply never fires this event and keeps the in-app modal.
  try {
    if (typeof window !== 'undefined') {
  ...
  } catch (e) { /* install prompt capture unavailable */ }
