...
async function measureHeader() {
  return await page.evaluate((cpFields) => {
    ...
    return {
      ...
      // ---- PDF Export "Use Company Profile" must survive --------------------
      exportCompany: (() => {
        const c = document.querySelector('#noteExportCompany');
        if (!c) return { exists: false };
        return { exists: true, type: c.type, label: (c.closest('label') || { textContent: '' }).textContent.trim() };
      })(),
      // ---- overflow ---------------------------------------------------------
...
await page.evaluate(() => {
  ...
  const ed = document.querySelector('#noteEditor') || document.querySelector('#fullScreenNoteEditor') || document.querySelector('.note-editor');
...
const previewRes = await page.evaluate(async () => {
  ...
  return {
    ...
    exportShown: !!document.querySelector('#noteExportPdfModal')?.classList.contains('show')
  };
});
...
check('no JavaScript errors during the whole run', pageErrors.length === 0, pageErrors.slice(0, 5).join(' | '));
...
process.exit(failCount === 0 ? 0 : 1);
  ...
  const modal = document.querySelector('#noteExportPdfModal');
  const chk = document.querySelector('#noteExportCompany');
  return {
    fired, dialogShown: !!modal?.classList.contains('show'),
    companyCheckbox: chk ? { exists: true, type: chk.type, label: (chk.closest('label') || {}).textContent?.trim() } : { exists: false }
  };
});
check('Export PDF button -> click handler ran', exportRes.fired === 1);
check('Export PDF button -> export dialog opens', exportRes.dialogShown);
check('PDF Export "Use Company Profile" option PRESERVED (shared code, kept)',
  exportRes.companyCheckbox.exists, JSON.stringify(exportRes.companyCheckbox));
await page.evaluate(() => document.querySelector('#noteExportPdfClose')?.click());
await sleep(300);

check('Save button -> click handler ran (saveCurrentOpenNote + closeFullScreenNote)',
  afterSave.fired === 1, `fired=${afterSave.fired}`);
check('Save button -> note persisted and editor closed',
  !afterSave.editorShown && afterSave.stored,
  `editorShown=${afterSave.editorShown} stored=${afterSave.stored} items=${afterSave.items} keysBefore=${beforeKeys.length} keysAfter=${afterSave.keys.length}`);
